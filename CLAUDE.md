...
1. **새 작업은 새 Claude Code 세션에서 시작한다.** 모델은 `/model opusplan`.
   이전 작업의 대화가 쌓인 세션에서 이어 가지 않는다 — 필요한 맥락은 `ClaudeICFR.md` 가 준다.
2. **STEP 0 은 plan 모드(Shift+Tab)에서 수행한다.** 실측·보고까지만 하고,
   마스터 확인을 받은 뒤 실행 모드로 전환해 구현한다.
3. **전체 테스트는 실패한 것만 보이게 돌린다** — `pytest -q`(백엔드), `npx vitest run --reporter=dot`(프론트) 등.
   통과 결과를 길게 출력하지 않는다. 실패가 있을 때만 그 부분을 펼쳐 본다.
4. **순차 2회 반복 검증은 마이그레이션이 있는 작업에만 한다.** 문서·화면 작업은 1회.
