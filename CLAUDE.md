...
│   ├─ adr/                ← ADR-0028부터 개별 파일 사용 시작. 분리 기준: 운영·인프라 및 대형 설계 ADR은 개별 파일(0028 인프라, 0029 cascade 시맨틱, 0030 baseline 테넌트 소유권, 0031 역할·권한 모델, 0032 평가 회차·워크플로, 0033 EUC·IUC 관리 모델, 0034 스코핑 모델, 0036 감사 컬럼 행위자), 코드 설계 ADR은 ClaudeICFR.md §10 요약 유지.
...
> 실행 명령·STEP 0/구현 단계 분리 여부·모델 선택은 §10 참조.
...
2026-09-22 마스터 지시. **Regina 도 같은 규칙을 따른다.**

1. **새 작업 전 세션 종료** — 이전 작업 세션은 `/exit`로 닫고 시작한다.
2. **필요 모델 표기** — 모든 프롬프트 파일은 맨 위에 "필요 모델"을 적는다.
3. **STEP 0 (Opus 1차 실행)**
...
5. **Opus 유지 예외** — 기존 데이터를 변경하는 마이그레이션, 권한·보안, 금액 계산 작업은
   구현까지 Opus로 진행한다. 이 경우 `--continue`에서도 `--model opus`를 사용한다.
6. **서버 명령은 마스터가 직접 실행** — 운영 서버·운영 DB 명령(백업, psql, docker, 배포 확인,
   시드 실행)을 Claude Code가 실행하지 않는다. 필요한 명령은 복사할 수 있는 코드 블록으로 제시하고,
   마스터가 결과를 붙여넣을 때까지 기다린다.
7. **마이그레이션 포함 작업 순서** — 마스터 백업 → push(PowerShell) → 배포 확인 → 필요 시 시드.
   각 단계의 명령은 6의 원칙에 따라 코드 블록으로 제시한다.
8. **문서만 변경하는 작업**은 STEP 0 없이 단일 실행한다.
9. **전체 테스트는 실패한 것만 보이게 돌린다** — `pytest -q`(백엔드), `npx vitest run --reporter=dot`(프론트) 등.
   통과 결과를 길게 출력하지 않는다. 실패가 있을 때만 그 부분을 펼쳐 본다.
10. **순차 2회 반복 검증은 마이그레이션이 있는 작업에만 한다.** 문서·화면 작업은 1회.
    2회를 돌릴 때도 반드시 순차로 한다(동시 실행 금지, `ClaudeICFR.md` 13.9-47).
