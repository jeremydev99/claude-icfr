...
44. **다음 작업은 EUC·IUC 구현이다(ADR-0033).** **4-3 역할 배정 화면은 뒤로 미뤘다.** 원천 양식(`backend/seeds/6__EUC_inventory.xlsx`·`7__IUC_inventory.xlsx`)과 13.9-43 의 양식 메모를 먼저 볼 것. 착수 전 짚을 것 — ①양식상 위험평가가 8건 전부 Low 라 §2.4 임계값 기준 통제 식별 대상이 0건이다 ②파일변경주기는 EUC 전용 상수 ③RCM `euc_description`·`related_systems`·`ipe_relevant` 와의 관계(ADR-0033 §3).
