...
2. source 뱃지(재정의/테넌트/기준) 실제 렌더링 — 미완
3. 생성/수정/삭제 버튼 비활성 + 툴팁 노출 UI — 미완
4. 브라우저 콘솔 envelope 계약 위반 에러 0건 (실 API 기준) — 미완
