...
4. **RCM baseline/overlay 후속 (ADR-0027)**: ~~2-A-1 병행 신규 구축~~ ✅ 완료 (2026-07-13). ~~2-B-1 상위 계층 baseline화 + baseline_controls.risk_id FK 전환~~ ✅ 완료 (2026-07-16, `ICFR_rcm_baseline_2b1_20260716.md`) — FK 체인 전역 복원으로 2-A-3 조회 전환 선행조건 해소. 다음: 2-A-2 기존 controls → baseline/instance 이관 → 2-A-3 조회 API 전환(resolve_controls 경유) → 2-A-4 CRUD 전환. 2-B-2 상위 계층 instance 테이블, 2-B-3 어서션 junction overlay, 2-B-4 resolver 확장.
