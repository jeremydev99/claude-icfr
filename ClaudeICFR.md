...
2026-08-06 baseline seed 작업 중 전체 pytest에서 확인. **커밋 85dc049(2-A-4-1) 시점부터 존재**하며 seed 파일을 제외해도 동일 재현 — 본 커밋과 무관하므로 섞지 않고 기록만 한다. 전체: 118 passed / 6 xfailed / 3 failed.

| 테스트 | 원인 | 처리 |
|---|---|---|
| `test_rcm.py::test_search_text` | **XPASS(strict)** — 2-A-4-1이 POST→instance(add) 전환으로 왕복을 복구시켰는데 `@_XFAIL_SRC_SPLIT` 마킹을 제거하지 않음 | 2-A-4-2에서 마킹 제거 |
| `test_rcm.py::test_bulk_delete` | bulk 삭제가 여전히 legacy `controls` 대상 (`assert 0 == 2`) | 2-A-4-2에서 instance 전환 |
| `test_rcm_crud_overlay.py::test_post_creates_add_instance` | 단독 실행은 통과, 전체 실행 시에만 실패 — 세션 스코프 공용 sqlite DB 오염(테스트 격리 문제) | 별도 수정 |
