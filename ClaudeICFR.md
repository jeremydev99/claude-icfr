...
| 10 | Phase 1 — A-1안 구현 | 🔄 진행중 (RCM·Test·Remediation·증빙·담당자/권한 FE 완료. BE+FE 사용자CRUD·비번·감사 일관화 완료. 공통 UI — 사이드바 디자인 개선·증빙 메뉴 평가 그룹 이동·UX 일관성 개선 완료. **멀티테넌시 근간(ADR-0025/0026) 1단계 완료** — tenant_id 전면·자동 격리. **2단계 완료(2026-07-08)** — 6개 테이블 (tenant_id, code) 복합 unique 전환. `/me` tenant 접근 정보 반환(2026-07-13)으로 **queryKey 팩토리 중앙화 완료(2026-07-13)** 선행 정리한 `X-Tenant-Id` 헤더 주입 재개 가능. **③ 완료(2026-07-14)** — auth store tenant 반영 + axios X-Tenant-Id 헤더 주입 + queryKey factory tenant 프리픽스, 멀티테넌시 후속 3항목 전부 완료. **RCM baseline/instance 2-A-1 완료(2026-07-13)** — 신규 테이블 2개 + resolve_controls 병행 구축(기존 controls 미변경). **2-B-1 완료(2026-07-16)** — baseline 상위 계층 4테이블(process/sub_process/risk/risk_category) + FK 체인 + baseline_controls.risk_id FK 전환. **2-B-2 완료(2026-07-16)** — instance 상위 계층 3테이블 + 이중 nullable FK(baseline/instance 상위 참조) + ControlInstance.risk_id 이중 FK 전환. **2-B-3 완료(2026-07-16)** — 어서션 junction baseline/overlay 2테이블(표준 연결 + 회사별 add/remove 결정). **2-B-3.5 완료(2026-07-16)** — baseline 5테이블 `baseline_version`(개정 회차, 행 단위) 컬럼 선반영 + 삭제 의미론 규약 문서화(baseline→exclude, add→soft delete). **2-B-4 완료(2026-07-16, 2-B 마지막)** — resolver 계층 확장: resolve_processes/sub_processes/risks + cascade(상위 exclude 전파) + 어서션 병합(코드 배열) + 관계 필드 + source envelope(id/source/baseline_id/is_overridden) 전 계층 공통 + 2-B-2 부채(risk_instance_id) 정리) | — |
