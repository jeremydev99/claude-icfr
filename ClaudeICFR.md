...
2. source 뱃지(재정의/테넌트/기준) 실제 렌더링
3. 생성/수정/삭제 버튼 비활성 + 툴팁 노출 UI
4. 브라우저 콘솔 envelope 계약 위반 에러 0건 (실 API 기준)
...
5. baseline/instance 데이터 백필 migration이 별도 예정 스텝인지(TrustBuilder 확인 필요)
6. resolver(`control_resolver.py`)가 `action`→`is_overridden` 매핑을 실제로 어떻게 제공하는지 — FE envelope 계약(`sourceEnvelope.ts`)은 `is_overridden` flat 필드를 전제하므로 코드 재확인 필요
