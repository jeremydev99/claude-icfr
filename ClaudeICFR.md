...
2. **3단계: 전체 모듈 관계도 (아키텍처)** — 섹션 2 채우기
   ...
   - 도메인 이벤트 흐름 (예: 운영평가 → 미비점 → 알림 → 개선계획 → 재테스트 → 종결)
3. **4단계: 개발 우선순위 및 로드맵** — 새 섹션(15) 신설
   - 모듈 의존성 기반 빌드 순서
   - MVP 범위 정의
   - 마일스톤별 산출물

### 13.3 후속 작업 (3·4단계 이후)
4. **기술 스택 결정 (ADR-0004)** — Frontend / Backend / DB / 인증 / 파일저장 / 알림
5. **백엔드 스켈레톤** — 모듈 구조, 공통 인프라(인증·감사로그·이벤트 버스)
6. **프론트엔드 스켈레톤** — 라우팅, 디자인 시스템, 공통 컴포넌트
7. **모듈별 구현** — 의존성 순서대로 (사용자/권한 → RCM → Scoping → 평가 → …)

### 13.4 데이터 모델 미결 사항 (섹션 5.7 참조)
