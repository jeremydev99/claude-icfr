...
| 10 | Phase 1 — A-1안 구현 | 🔄 진행중 (RCM·Test·Remediation·증빙·담당자/권한 FE 완료. BE+FE 사용자CRUD·비번·감사 일관화 완료. 공통 UI — 사이드바 디자인 개선·증빙 메뉴 평가 그룹 이동·UX 일관성 개선 완료. **멀티테넌시 근간(ADR-0025/0026) 1단계 완료** — tenant_id 전면·자동 격리. **2단계 완료(2026-07-08)** — 6개 테이블 (tenant_id, code) 복합 unique 전환. `/me` tenant 접근 정보 반환(2026-07-13)으로 **queryKey 팩토리 중앙화 완료(2026-07-13)** 선행 정리한 `X-Tenant-Id` 헤더 주입 재개 가능. **③ 완료(2026-07-14)** — auth store tenant 반영 + axios X-Tenant-Id 헤더 주입 + queryKey factory tenant 프리픽스, 멀티테넌시 후속 3항목 전부 완료. **RCM baseline/instance 2-A-1 완료(2026-07-13)** — 신규 테이블 2개 + resolve_controls 병행 구축(기존 controls 미변경). **2-B-1 완료(2026-07-16)** — baseline 상위 계층 4테이블(process/sub_process/risk/risk_category) + FK 체인 + baseline_controls.risk_id FK 전환. **2-B-2 완료(2026-07-16)** — instance 상위 계층 3테이블 + 이중 nullable FK(baseline/instance 상위 참조) + ControlInstance.risk_id 이중 FK 전환. **2-B-3 완료(2026-07-16)** — 어서션 junction baseline/overlay 2테이블(표준 연결 + 회사별 add/remove 결정). **2-B-3.5 완료(2026-07-16)** — baseline 5테이블 `baseline_version`(개정 회차, 행 단위) 컬럼 선반영 + 삭제 의미론 규약 문서화(baseline→exclude, add→soft delete). **2-B-4 완료(2026-07-16, 2-B 마지막)** — resolver 계층 확장: resolve_processes/sub_processes/risks + cascade(상위 exclude 전파) + 어서션 병합(코드 배열) + 관계 필드 + source envelope(id/source/baseline_id/is_overridden) 전 계층 공통 + 2-B-2 부채(risk_instance_id) 정리. **2-A-2 이관 완료(2026-07-21)** — 기존 RCM 699행 → baseline 6테이블 복사(instance 0건=암묵 adopt, 원본 미변경). **2-A-3 완료(2026-07-23)** — `/controls/search`·`/controls/{id}` resolve_controls 기반 전환 + envelope flat 추가. 라이브 95건·필터/정렬/페이지네이션 legacy 동일·상세 200/404 검증. **단 test_rcm.py 7건 xfail(strict)** — POST(legacy)→read(resolver) 소스 분리, 2-A-4에서 복구 예정. **baseline seed 구축 완료(2026-08-06)** — 엑셀 단일 원천(`backend/seeds/`) + `seed_baseline.py`로 전 환경 동일 baseline 재현. 실측: 엑셀 93 통제(8~100행), 현행 baseline 95 = 동일 93 + 데모 시드 P-SALES 2건, 공통 93건 내용·어서션 **차이 0건**. `--reset` 실행 완료(마스터 수행) — baseline 93 통제, 구 `controls` 95 불변, resolve 93건, instance 0건 검증. **2-A-4-2 완료(2026-08-11)** — bulk 2개 + `GET /controls` 전환 + `clear-all` 제거로 통제 계층 읽기·쓰기 전 경로 baseline/overlay 통일. pytest 128 passed/6 xfailed/0 failed). **2-A-4-3 완료(2026-08-24)** — 상위 3계층 조회·CRUD resolver/overlay 전환 + cascade 시맨틱 ADR-0029 채택, 운영 확인 완료. **2-A-4-4 완료(2026-09-01)** — 어서션 junction CRUD(`POST`/`DELETE`/`GET /control-assertions`)를 `control_assertion_instances` 로 전환, `GET /risk-categories` 읽기 2개를 baseline 으로 전환. **RCM 통제 계층의 읽기·쓰기가 어서션까지 전부 baseline/overlay 로 통일됨.** pytest 161 passed/1 xfailed/0 failed(잔여 xfail은 upload-excel 1건). **13.9-10-a 완료(2026-09-01)** — upload-excel 멀티헤더 0건 파싱 수정(엔드포인트 실측 0→93건) + 0건을 성공으로 반환하지 않도록 422 처리 + 멀티헤더 회귀 테스트 6건 신설. pytest 167 passed/1 xfailed/0 failed(잔여 xfail `test_excel_upload_commit` 은 13.9-10-b 쓰기 경로 소속). 로컬 검증만 완료·운영 미확인) | — |
...
| Scoping | ✅ ADR-0034 (+§6 정정) | ✅ | ✅ | ✅ 6-1 전역 템플릿(192계정)·회계연도 복사·필드 단위 배지·조회 시 산출·상태 전이·확정 스냅샷 + **6-1b 검토 확인(`confirmed`)·중요성 기준 회계연도별 회사 설정(가이드 범위·설정율 범위·질적 기준·기준 연도)·스냅샷에 기준값 전부** | ✅ 중요성 기준·조정 항목·계정 평가(기준/전년 금액·증감률)·줄/영역/문구 단위 확인·수동 판정·검토 기록·가이던스 + 대시보드 카드 | ✅ 37건 | 13.9-50·53. 6-1 운영 배포됨. 6-1b 마이그레이션 `b1c2d3e4f5a6` push 대기(로컬 검증만). 6-2(GITC·프로세스 연결)·재무제표 추출 ADR 남음 |
| EUC | ✅ | ✅ | ✅ | ✅ 5-1 파일 CRUD·위험 산출·임계값 정책·원천 시드 | ✅ 파일 중심 목록·등록·수정(행별 `can_edit`)·원천 참고값 비교 | ✅ 26건(EUC·IUC 공통) | ADR-0033 정정본. 5-2(통제 4종·회차) 남음. 로컬 검증만 |
| IUC | ✅ | ✅ | ✅ | ✅ 5-1 정보 항목 CRUD(통제 단위 권한)·effective 제외 | ✅ 통제 중심 목록·등록·수정 | ✅ (EUC 와 공통) | 통제 FK 없음 — 13.9-27 위험 승계. 로컬 검증만 |
...
| 감사 컬럼 | **로컬 커밋, push 대기 (2026-09-22, ADR-0036)** — 배포되면 새 쓰기부터 `created_by`·`updated_by`·`deleted_by`·`deleted_at` 이 채워진다. 기존 행은 NULL 그대로 | 13.9-51 |
...
3. **멀티테넌시 후속 (ADR-0026 미결)**: ~~①`code` 등 글로벌 unique → `(tenant_id, code)` 복합 unique 전환~~ ✅ 완료 (2026-07-08, ADR-0026 추가결정 참조). ②tenant 생성/온보딩 API + 회사 전환 UI — **미착수 (다음 세션 후보)**. ~~③프론트엔드 `X-Tenant-Id` 헤더 주입~~ ✅ 완료 (2026-07-14, `ICFR-PROMPT-tenant-header-axis.md`) — auth store tenant 반영 + axios 인터셉터 헤더 주입 + queryKey factory tenant 프리픽스. tenant 1개뿐인 현재는 동작 변화 없음(구조만 확보). ~~③-선행: queryKey 팩토리 중앙화~~ ✅ 완료 (2026-07-13).
4. **RCM baseline/overlay 후속 (ADR-0027)**: ~~2-A-1 병행 신규 구축~~ ✅ 완료 (2026-07-13). ~~2-B-1 상위 계층 baseline화 + baseline_controls.risk_id FK 전환~~ ✅ 완료 (2026-07-16, `ICFR_rcm_baseline_2b1_20260716.md`) — FK 체인 전역 복원으로 2-A-3 조회 전환 선행조건 해소. ~~2-B-2 상위 계층 instance 테이블 + 이중 FK~~ ✅ 완료 (2026-07-16, `ICFR_rcm_baseline_2b2_20260716.md`) — 계층 참조 방식(이중 nullable FK) 통일. ~~2-B-3 어서션 junction overlay~~ ✅ 완료 (2026-07-16, `ICFR_rcm_baseline_2b3_20260716.md`) — 표준 연결 + 회사별 add/remove 2-action 결정. ~~2-B-3.5 baseline_version + 삭제 의미론~~ ✅ 완료 (2026-07-16, `ICFR_rcm_baseline_2b35_20260716.md`) — baseline 5테이블 개정 회차 컬럼(행 단위) 선반영 + 삭제 규약 문서화. ~~2-B-4 resolver 계층 확장~~ ✅ 완료 (2026-07-16, `ICFR_rcm_baseline_2b4_20260716.md`, **2-B 완료**) — 상위 계층 resolve 3함수 + cascade + 어서션 병합 + 관계 필드 + source envelope + 2-B-2 부채 정리. 2-A-3 조회 전환 선행조건 전부 해소. 다음: 2-A-2 이관 → 2-A-3 조회 API 전환(resolve_controls 경유) → 2-A-4 CRUD 전환 → 2-A-2 기존 controls → baseline/instance 이관 → 2-A-3 조회 API 전환(resolve_controls 경유) → 2-A-4 CRUD 전환. ~~FE 어댑터 경계 도입~~ ✅ 완료 (2026-07-20, `ICFR-PROMPT-rcm-adapter-layer.md`) — controlsApi/rawcApi 응답 타입을 DTO로 분리, 2-B-4 계약 확정 시 컴포넌트·훅 무변경으로 흡수 가능. ~~FE envelope 흡수 자리 준비~~ ✅ 완료 (2026-07-22, `ICFR-PROMPT-envelope-adapter-prep.md`) — SourceEnvelope 타입·4계층 optional 필드·adapter 매핑 준비 후, 원격 2-B-4(d325705) 실 구현이 envelope를 flat 필드(source/baseline_id/is_overridden, id는 기존 필드 재사용)로 얹힌 것을 확인해 DTO를 flat으로 정합(어댑터가 도메인 nested SourceEnvelope로 조립). ~~2-A-2 이관~~ ✅ 완료 (2026-07-21, `ICFR_rcm_migrate_2a2_20260721.md`, 커밋 33039f0) — 699행 baseline 복사, instance 0건, 원본 미변경. ~~2-A-3 조회 API 전환~~ ✅ 완료 (2026-07-23, `ICFR_rcm_search_2a3_20260723.md`) — search·상세를 resolve_controls 경유로 전환 + envelope flat. 라이브 95건·필터 legacy 동일 검증. ~~2-A-3-1 envelope required 전환(조회 계층, control 도메인)~~ ✅ 완료 (2026-08-13, `ICFR-PROMPT-2A3-1-envelope-required.md`) — control 계층 한정, 상위 3계층은 백엔드 resolver 미배선으로 제외(13.7 참조). 다음: **2-A-4 CRUD 전환**(POST/PATCH/DELETE → instance adopt/exclude/override/add).
   - **2-A-4 미결 (필수)**: test_rcm.py 7건 xfail(strict=True) 상태 — write(legacy controls POST)→read(resolver) 소스 분리로 왕복 테스트가 red. **2-A-4에서 POST→instance(add) 전환 시 왕복 복구되어 xpass → xfail 마킹 제거해야 함**(strict라 복구 시 실패로 떠서 자동 신호). 마커: `_XFAIL_SRC_SPLIT`(tests/test_rcm.py).
   - ~~2-A-4-2 다건·목록 전환~~ ✅ 완료 (2026-08-11, `ICFR_rcm_bulk_2a42_20260807.md`) — `bulk-delete`/`bulk-update`/`GET /controls` 전환 + `clear-all` 제거 + 분기 함수 추출(`_apply_control_update`/`_apply_control_delete` — 단건·다건 공용, 커밋은 호출자). **통제 계층의 모든 읽기·쓰기 경로가 baseline/overlay로 통일됨.** bulk-update는 `exclude_none`→`exclude_unset` 통일(False/`""` 저장 불가 문제 해소). 다음: **2-A-4-3**(상위 계층 processes/sub-processes/risks + 어서션 junction CRUD 전환, upload-excel 파서 코어 분리·다중 헤더행 대응) — 남은 xfail 6건이 여기서 해소된다.
   - ~~baseline seed 구축~~ ✅ 완료 (2026-08-06, `ICFR_baseline_seed_20260723.md`) — 엑셀 단일 원천 repo 포함 + `seeds/seed_baseline.py`(재실행 가드·`--reset`·단일 트랜잭션) + `tests/test_seed_baseline.py` 8건 + Dockerfile `COPY seeds/`·`COPY scripts/`. 2-A-2 이관 스크립트는 **역할 종료**(repo 잔존, 실행하지 않음) — seed가 정식 경로. 실행(`--reset`)은 마스터 직접 수행.
5. Phase 1.5 → 2 → 3 단계적 확장

### 13.5 기존 테스트 실패 3건 — ✅ 2-A-4-2에서 전부 해소 (2026-08-11)
   - ~~baseline seed 구축~~ ✅ 완료 (2026-08-06, `ICFR_baseline_seed_20260723.md`) — 엑셀 단일 원천 repo 포함 + `seeds/seed_baseline.py`(재실행 가드·`--reset`·단일 트랜잭션) + `tests/test_seed_baseline.py` 8건 + Dockerfile `COPY seeds/`·`COPY scripts/`. 2-A-2 이관 스크립트는 **역할 종료**(repo 잔존, 실행하지 않음) — seed가 정식 경로. ~~실행(`--reset`)은 마스터 직접 수행~~ ✅ **Regina 로컬 실행 완료 (2026-08-11)** — `docker compose exec backend python -m seeds.seed_baseline --reset`(모듈 실행. 스크립트 직접 실행은 `app` 패키지 임포트 깨짐 확인). 삭제 전 baseline+instance 11테이블 전부 0행(실데이터 손실 없음) → 엑셀 재시딩 → baseline_processes 8·baseline_sub_processes 29·baseline_risks 85·baseline_risk_categories 7·baseline_controls 93·baseline_control_assertions 469, instance 5테이블 0(암묵 adopt), legacy `controls` 98 불변 — 스크립트 출력과 `docker compose exec postgres psql` 직접 카운트 쿼리 2회(baseline 6개/instance+legacy 6개, `-U icfr_user -d icfr_db`)로 교차검증 일치.
5. Phase 1.5 → 2 → 3 단계적 확장

### 13.4 대기 목록 (미검증 — 2-A-2 이관 확인 후)

FE envelope required 전환(2026-07-24, `ICFR-PROMPT-envelope-required.md`)은 코드·타입·빌드 레벨까지만 검증됨. 로컬 DB의 `baseline_controls`/`baseline_control_assertions`/`control_instances`/`control_assertion_instances`가 0건(문서상 2-A-2 699행 이관 기록과 불일치)이라 아래는 육안·라이브 검증 보류:

1. 컨트롤 목록 라이브 카운트 95건 / `process_code=EL` 37건 / `assertion=E` 73건 / `risk_level=SR` 1건
   - ~~1-a. DB baseline 원천 데이터 검증~~ ✅ **완료 (2026-08-11)** — `psql` 직접 카운트 교차검증, `baseline_controls` **93건**(기준값 95는 무효, 엑셀 원천 93이 정답). 13.4 하단 신규 항목5 참조.
   - 1-b. 라이브 API 카운트 검증(`GET /controls/search` 실호출, `process_code=EL`/`assertion=E`/`risk_level=SR` 필터별 건수) — **`process_code` 부분 완료 (2026-08-12, `ICFR-PROMPT-process-filter-dynamic.md`)**: 브라우저 RCM 필터 UI에서 8개 프로세스 코드 전수 노출 확인 + 93-기반 기대 분포(EL 37/EX 3/FA 7/FR 12/HR 10/IT 2/SD 8/TR 14) 중 `EL` 37·`FA` 7·`TR` 14 실측 일치. **`assertion`/`risk_level` 부분은 이번 세션 범위 밖 — 여전히 미완**, 93 기반 기대값 재산정 후 별도 확인 필요.
2. ~~source 뱃지(재정의/테넌트/기준) 실제 렌더링~~ ✅ **완료 (2026-08-13, control 계층 한정)** — envelope required 전환(`ICFR-PROMPT-2A3-1-envelope-required.md`) 브라우저 검증에서 SourceBadge 정상 렌더링 확인. 상위 3계층은 미검증(해당 계층 envelope 자체가 아직 optional, 13.7 참조).
3. ~~생성/수정/삭제 버튼 비활성 + 툴팁 노출 UI~~ ✅ **해소 (2026-08-14, `ICFR-PROMPT-2A3-2-mutation-ui-wiring.md`)** — mutation 잠금 자체를 물리 제거(2-A-3-2), 버튼 활성화 + 실 API 왕복(생성/편집/삭제) 브라우저 검증 완료. 잠금은 Excel 업로드 1곳(`EXCEL_UPLOAD_LOCKED`, 13.6 미해소)만 남음.
4. ~~브라우저 콘솔 envelope 계약 위반 에러 0건 (실 API 기준)~~ ✅ **완료 (2026-08-13, control 계층 한정)** — 동일 검증에서 콘솔 envelope 계약 위반 에러 0건 확인(React Router 경고만 존재, 무관).

DB 이관(2-A-2 재실행)은 백엔드(TrustBuilder) 소관 — 본 세션(Regina/Claude FE)에서 실행하지 않음. 이관 확인 후 위 4항목 재검증 필요.

**2026-08-04 재확인 (read-only DB 조회, Regina/Claude)**: 위 4테이블 재조회 — 여전히 0건(변화 없음). 스키마 자체(baseline 6 + instance 5 = 11테이블)는 전부 존재 확인 — **2-A-2 스키마 랜딩은 완료, 데이터 백필만 미완**으로 구분해 기록. `control_instances` 컬럼 전수 확인 결과 overlay 판별은 `baseline_control_id`(nullable)+`action`(NOT NULL) 조합이며, **`is_overridden` 물리 컬럼은 존재하지 않음**(2-B-4 changelog가 서술하는 envelope의 `is_overridden`은 DB 컬럼이 아니라 resolver 계산 결과로 추정 — 코드 미열람이라 미확정).
...
2026-08-06 baseline seed 작업 중 확인한 3건(커밋 85dc049 이후 존재). 2-A-4-2에서 해소됐고, 현재 전체 pytest는 **128 passed / 6 xfailed / 0 failed**.

| 테스트 | 원인 | 결과 |
|---|---|---|
| `test_rcm.py::test_search_text` | XPASS(strict) — 2-A-4-1이 왕복을 복구시켰는데 마킹을 제거하지 않음 | ✅ 마킹 제거 |
| `test_rcm.py::test_bulk_delete` | bulk 삭제가 legacy `controls` 대상 (`assert 0 == 2`) | ✅ instance 전환으로 복구 |
| `test_rcm_crud_overlay.py::test_post_creates_add_instance` | **초기 진단(공용 sqlite 오염)은 부정확했다.** 실제 원인은 `test_rcm_baseline.py`의 `ControlInstance(action="add")` 픽스처가 `activity_*` 6 + `is_key_control`/`preventive_detective`/`auto_manual`/`frequency`/`ipe_relevant` 5 = **11개 필드를 비워둔 것**. resolver가 add 행의 NULL을 그대로 내보내 이후 모든 `search`/`GET /controls` 호출이 `ControlSearchOut` 검증에서 깨졌다(adopt/override는 baseline이 NOT NULL이라 무관). | ✅ 픽스처를 규약대로 수정 |

**중요 — 제품 버그가 아니다.** `models/rcm_baseline.py`의 `ControlInstance` 규약이 "add: 자체 필드 **전부 채움**"이고, API 경로는 `ControlBase` 기본값(`activity_*=False`, `is_key_control=True`, `P`/`M`/`A`/`N/A`)으로 항상 채운다. 규약을 어긴 것은 ORM으로 직접 행을 만드는 테스트 픽스처뿐이었다. **제품 코드 무변경**으로 해소.

> 교훈: instance 테이블은 미러링 때문에 전 컬럼이 nullable이라 DB가 이 규약을 강제하지 못한다. add 행을 코드로 직접 만들 때(마이그레이션·임포트·픽스처)는 필수 필드를 채워야 한다.

### 13.5.1 xfail 현황 (6건 → **1건**, 전부 `_XFAIL_SRC_SPLIT`)

2-A-4-2 후 남은 xfail은 전부 **동일 근본원인** — write가 legacy로 가는데 read는 resolver를 보는 소스 분리. 2-A-4-3에서 4건, **2-A-4-4에서 1건**이 해소되어 `test_excel_upload_commit` 1건만 남았다(13.9-10).
...
`_parse_rcm_sheet`는 `header_row + 1`을 데이터 시작행으로 가정한다. 그런데 최종 엑셀은 헤더가 **6행(주헤더) + 7행(2차 헤더)** 2단이고 데이터는 8행부터라, 7행에서 `row[p_col] is None` → 즉시 `break` → **0건 파싱**된다. 즉 **현재 `POST /api/rcm/upload-excel`에 이 엑셀을 올리면 0건**이다.
...
**2026-09-01 수정 완료** (`prompts/ICFR_excel_fix_20260901.md`, 커밋 3건). `_find_data_start` 를 `services/excel_parser.find_data_start_row` 로 공용화하고 `upload-excel` 호출부가 seed 와 **같은 함수**로 시작행을 찾게 했다(복제하면 한쪽만 고쳐진다 — 이 결함이 정확히 그 형태였다). **엔드포인트 경유 실측: 0건 → 93건.** 함께 **0건을 성공으로 반환하지 않도록** 막았다(422 + `status: "no_data_rows"`, 기존 `header_not_found` 와 동일 규약) — 원인과 무관하게 다른 파일이 0건이어도 "업로드 성공"으로 보이는 일이 없어야 한다. 회귀 테스트 6건 신설(`tests/test_rcm_excel_multiheader.py`, 합성 픽스처 + 실파일). **로컬 검증만 완료 — 운영 미확인.** `EXCEL_UPLOAD_LOCKED` 해제는 13.9-10-b(쓰기 경로 overlay 미전환) 이후 판단.

### 13.7 부수 발견 — processes 엔드포인트 resolver 미전환 (기록만)

`GET /api/rcm/processes`는 2-B-4에서 구축된 `resolve_processes` 병합 경로가 아니라 레거시 `processes` 테이블을 직접 조회한다(`ICFR-PROMPT-process-filter-dynamic.md` 작업 중 확인). 현재 tenant 1개·`process_instances` 전부 0건(암묵 adopt)이라 실질 응답 차이는 없다. 향후 processes도 tenant add/override가 발생(2-B급 전환)하면, 이 엔드포인트도 `resolve_processes` 기반으로 갱신되어야 프로세스 필터(및 2-A-4 상위 계층 CRUD)에 정확히 반영된다. 코드 변경 없음, 기록만.
...
> **운영 규칙 — push 승인의 단위 (2026-09-01 확립)**
>
> **push 정책은 커밋 단위가 아니라 브랜치 단위로 적용된다.**
> push 대기 커밋이 존재하면 이후 모든 커밋의 push가 함께 승인되는 것과 같다.
> **대기 커밋이 있는 상태에서는 문서 커밋도 자동 push 대상이 아니다.**
>
> 배경 — 2026-09-01 ADR-0030 커밋 시 "문서 변경이라 자동 push 승인"으로 진행했으나,
> 그 시점 `main` 에는 13.9-10-a 코드 커밋 4건이 push 대기 상태로 쌓여 있어 함께 올라갔다
> (`7c30290`·`ffdea99`·`e78c8f4`·`4131d73`). 되돌리지 않기로 결정했다 — Regina 공유는
> 이미 완료(회신 대기 중일 뿐)였고, 변경 방향이 **잘못된 성공을 실패로 바꾸는 쪽**이라
> 안전하며, `EXCEL_UPLOAD_LOCKED` 로 FE 진입도 막혀 있다. revert 는 방금 고친 결함을
> 운영에 되돌리는 일이 된다.
>
> 원인은 지시가 부정확했던 것이지 실행 오류가 아니다. **"문서만 자동 push"라는 지시는
> 대기 커밋이 있는 상태에서 성립하지 않는다.** 앞으로 push 승인 전에는 대기 커밋 유무를
> 먼저 확인하고, 있으면 그 사실과 함께 승인 범위를 다시 확인한다.
...
8. **용어·제도 해설 기능 → 매뉴얼 패널** (백로그 등록 2026-08-24, **2026-09-22 범위·배치·구조·순서 재결정 — 아래 정정 이력**) — ICFR 담당자 다수가 회계 비전문가라 어서션·통제유형·설계평가 등 용어를 모른 채 입력 판단을 해야 한다. **비전문가가 전문가처럼 사용하는 것**이 목표.
   - 범위: (1) 용어 사전 데이터 (2) 마우스오버 툴팁 (3) 별도 매뉴얼 페이지 (4) 관련 규정 연동 및 중요부분 강조 표시
   - ~~**배치 확정 (2026-08-24): 화면 우측에서 슬라이드로 열리는 패널.** 읽기 흐름과 방향이 같고, 본문 **위에 겹쳐** 뜨므로 작업이 끊기지 않는다.~~ → **2026-09-22 변경(아래): 본문을 밀어내는 방식 + 좁은 화면은 겹침**
   ...
   - **2026-09-22 정정 — 범위·배치·구조·콘텐츠·순서 재결정** (위 2026-08-24 서술은 기록으로 남긴다)
     - **범위 확장**: 용어 해설만이 아니라 **모든 메뉴·기능·항목에 매뉴얼을 붙인다.** 용어를 몰라서 막히는 것과 화면 동작을 몰라서 막히는 것은 사용자에게 같은 문제다. 5-1·6-1 로 화면이 늘면서 "이 배지가 무슨 뜻인가"(미평가·템플릿 문구·원천 참고값) 같은 기능 설명의 필요가 용어 설명만큼 커졌다.
     - **배치 변경**: 9월 초 결정(본문 위에 겹침)을 바꿔 **우측 패널이 열리면 본문이 밀린다.** 겹치면 패널이 설명하는 바로 그 항목을 가리게 되어, 매뉴얼을 보면서 입력하는 흐름이 끊긴다. 대신 **패널 폭은 사용자가 조절**할 수 있고, **화면이 좁을 때는 겹침으로 전환**한다 — RCM 표처럼 열이 많은 화면이 밀려서 망가지지 않게 하려는 것이며, 08-24 에 사이드바 옆 배치를 기각한 이유(표 열이 잘려 결국 닫아두게 된다)를 그대로 존중한다. 전환 폭 기준은 패널 틀 작업에서 정한다.
     - **구조 — 문구를 화면 코드에 넣지 않는다.** 메뉴·기능·항목에 **키**를 붙이고, 설명은 그 키로 **따로 저장**한다. **용어 사전과 같은 저장소**로 둔다(용어와 매뉴얼을 두 곳에 두면 같은 말이 서로 다르게 설명된다). 문구가 코드에 있으면 문구 하나 고치는 데 배포가 필요하고, 콘텐츠 담당(마스터)이 코드를 거쳐야 한다 — 위 분담 원칙과도 어긋난다.
     - **콘텐츠 — ADR 문구를 그대로 쓰지 않는다.** ADR 은 개발자용이라 "effective 제외"·"정체성 id" 같은 말이 사용자에게는 의미가 없다. **사용자 말로 따로 작성**한다. 규정은 위 원칙 그대로 **출처·조번호와 함께 요지만**, **기준일을 표기**한다.
     - **순서 — 6-1 완료 후, 6-2 착수 전에 패널 틀과 키 규칙을 먼저 만든다.** **6-2 부터 새 화면은 처음부터 키를 단다.** 기존 화면(RCM·EUC·IUC·스코핑·대시보드·관리자 기능 등)은 **키를 일괄로 붙이고 문구는 점진적으로** 채운다. 키 규칙이 먼저 있어야 화면마다 제각각 키를 만드는 일을 막을 수 있고, 뒤에 붙이면 기존 화면 전체를 다시 훑어야 한다.
     - 선행 조건 재정리: 08-24 의 "콘텐츠가 먼저 있어야 UI 작업 가능"은 **틀·키 규칙은 콘텐츠 없이 먼저 만들 수 있고, 콘텐츠가 없는 키는 빈 상태로 둔다**로 바꾼다. 틀이 없으면 콘텐츠를 어디에 어떤 키로 쓸지 정할 수 없어 오히려 콘텐츠 작업이 막힌다.
...
11. ~~**2-A-4-3 (부분 완료)**~~ ✅ **완료 (2026-09-04)** — 상위 3계층 조회·CRUD 전환은 백엔드(위 6번, `cad62a9`) + 프론트(14번, 아래) 모두 완료. **남은 범위**: upload-excel 파서 분리(10번)뿐. 어서션 junction CRUD(9번)는 2026-09-01 완료. 남은 xfail 1건은 10번에 붙어 있다.
12. **코드마스터 테이블화** — 미착수. **ADR-0030 §6 미해결과 연결된다** — 코드 체계가 테넌트별이면(§2.2 `(tenant_id, code)` 전환) 코드마스터도 테넌트별인지 미결이다. 16번 결정에 따라 이 항목의 설계가 갈린다.
13. **Regina NCP Sub Account** — `icfr-regina` / `icfr-view-only` 정책(ADR-0028 §2.7) 발급 미완.
14. ~~**상위 3계층 CRUD 프론트 배선**~~ ✅ **완료 (2026-09-04, `prompts/rcm_상위3계층_crud_ui.md`, 커밋 `d2c4002`)** — RCM 모듈에 "계층 관리" 신규 탭 추가(15-1 결정: RCM 내부, 별도 라우트 아님). Process/SubProcess/Risk 각 목록 + 생성(상위 드롭다운 전용, 자유입력 금지) + 편집(code·상위참조 read-only) + 삭제(Control의 exclude/soft-delete 패턴 재사용, `HierarchyDeleteConfirmDialog` 공용화). 신규: `hierarchyApi.ts`/`useHierarchy.ts`(controlsApi/useControls 미러링), `errors.ts`(`extractHierarchyErrorMessage` — 3계층 공통 단일 409 핸들러, 서버 detail 그대로 노출), `permissions.ts`(`canEditHierarchy()` — 권한 seam, ~~현재 무조건 true, ADR-0031 미확정~~ ✅ **완료 (2026-09-09)** — `/me` `can_write` 위임, 상세는 §14 2026-09-09 항목 참조), `components/hierarchy/` 6개(FormDialog×3·Table×3). `controlsAdapter.ts`에 단일 어댑터(`toProcess`/`toSubProcess`/`toRisk`) 추가, `ProcessItem`에 `description` 필드 보강. `docs/api/rcm-hierarchy-contract.md`는 저장소에 실존하지 않아(어느 브랜치에도 커밋 이력 없음) 백엔드 소스(`api/rcm.py`/`schemas/rcm.py`)를 직접 근거로 사용. `tsc` 0 errors·`npm run build` 통과·vitest 27/27(신규 10건, 어댑터·에러핸들러·권한 seam 순수 로직). **로컬 검증만 완료 — 브라우저 실 API 왕복·운영 미확인, push 대기.**
15. ~~**미결 결정 2건**~~ ✅ **전부 해소** — 1) UI 위치: RCM 모듈 내부 신규 탭으로 확정(14번 완료 반영). 2) `envelope` required 전환: 2026-09-02 별도 작업으로 이미 완료(위 12.2 RCM 행 참조).
...
**API 계약 결함 (2026-09-03 등록 — 상위 3계층 CRUD 계약 정리 중 발견, `docs/api/rcm-hierarchy-contract.md`)**

17. ~~**code 중복 검증이 통제에만 없다**~~ ✅ **해소 (2026-09-03, 커밋 `f3b0f29`)** — `create_control` 에 `_assert_code_available(db, BaselineControl, ControlInstance, body.code, "통제")` 추가. 호출 형태는 상위 3계층과 동일하고 헬퍼는 무변경. 실측 응답 — `409 {"detail": "통제 코드 'X' 는 표준(baseline)에 이미 있습니다"}` / `409 {"detail": "통제 코드 'X' 는 이미 사용 중입니다"}` 로 **상위 3계층과 구조 동일(라벨만 다름)** — FE 가 409 처리를 4계층 공통 코드로 통합할 수 있다. 회귀 테스트 3건(`test_rcm_crud_overlay.py`). `docs/api/rcm-hierarchy-contract.md` 를 같은 커밋 흐름에서 갱신했다(§5 표·§6-① 해소 이력·상단 경고 제거·스냅샷 해시 `f3b0f29`). **로컬 검증만 완료 — 운영 미확인.** 아래는 등록 당시 기록.

    **원 증상**:  **통제 생성 시 baseline과 code가 겹쳐도 검증 없이 201로 생성된다. resolver 결과에 같은 code가 둘 나타난다. 에러가 나지 않아 발견이 늦다.** 원인: `_assert_code_available`이 2-A-4-3(`cad62a9`)에서 신설되며 상위 3계층에만 적용되고 `create_control`(`rcm.py:692`)로 소급되지 않음. 상위 3계층은 409 + 구분된 한국어 메시지("표준(baseline)에 이미 있습니다" / "이미 사용 중입니다")를 반환하는데 통제만 다르다. 통제는 instance끼리 겹칠 때만 DB 유니크 위반으로 전역 핸들러의 409(일반 문구 "데이터 무결성 제약 위반")가 나가고, **baseline과 겹치는 경우는 어떤 제약도 막지 못한다**(서로 다른 테이블이라 제약이 걸치지 않음 — ADR-0029 §3에 기록된 사각지대와 같은 구조). 조치: `create_control`에서 `_assert_code_available(db, BaselineControl, ControlInstance, body.code, "통제")` 호출. **수정 시 `docs/api/rcm-hierarchy-contract.md` §5 표와 §6-①을 함께 갱신할 것** — 그 문서가 이 결함을 현재 동작으로 기술하고 있다. FE 영향: 통제 생성 폼과 상위 3계층 폼의 409 처리를 같은 코드로 쓸 수 없다.

18. **낙관적 잠금이 API 계약에 없다 (4계층 공통)** — `row_version` 컬럼은 모델(`VersionMixin`)에 있으나 `schemas/rcm.py`·`api/rcm.py`에 **언급이 0건**이다. 요청에 버전을 실을 곳도, 충돌 409를 받을 경로도 없다. **동시 편집 시 마지막 쓰기가 이긴다.** 현재 사용자 2명이라 발현 가능성이 낮으나, **고객사 다중 사용자 환경에서는 감사 대응상 문제가 될 수 있다** — 누가 무엇을 덮어썼는지 증적이 남지 않는다. 온보딩 전 판단 필요. 조치 방향(미결): PATCH 요청에 `row_version`을 받아 불일치 시 409 반환. 4계층 동시 적용이어야 하고 FE 재배선이 따르므로 별도 설계.

**설계 결정 (2026-09-03 등록)**

19. **역할·권한 모델 확정 (ADR-0031, 제안(초안))** — 증빙 관리·Test 모듈 설계 중 **통제별 담당자 배정 체계가 없다**는 선행 문제를 발견해 작성. 현재 권한은 admin/일반 2단계뿐이고 RCM 담당자 컬럼은 계정과 연결되지 않은 문자열이다. 결정 요지: 역할 8종(`control_owner`/`dept_approver`/`assessor`/`icfr_manager`/`ceo`/`auditor`/`external_auditor`/`sys_admin`), **역할은 사람이 아니라 통제 단위로 배정**(중소기업은 같은 사람이 통제 A의 책임자·통제 B의 평가자가 되는 상호 배정이 불가피), 배정 단위는 **프로세스 기본값 + 통제별 예외**(baseline/overlay와 같은 구조 — 저장은 차이만, 조회는 해석), 이해상충은 **막지 않고 경고+사유 기록**(막으면 중소기업이 못 쓰고, 조용히 허용하면 감사에서 지적된다. 사유 기록이 보완통제 증적이 된다). 부서는 §2.8 — **회사 조직도의 스냅샷**이며 원본은 인사시스템, 계층은 `parent_id` 컬럼으로 개념만 보존하고 평면 구현(스키마 변경은 횟수 자체가 위험 — ADR-0030 경험). ~~**후속 ADR-0032 예정**~~ ✅ **작성 완료 (2026-09-03, 21번)** — ADR-0031 §5의 **"평가 회차별 역할 변동"과 §2.8의 "당시 부서장이 누구였는가" 두 미해결이 ADR-0032 §2.8로 해소**됐다. 해법은 **배정 이력을 관리하지 않고 수행 기록에 수행자를 남기는 방식** — 감사에서 묻는 것은 "1분기 평가를 누가 했는가"이고 그 답은 평가 기록에 있다. 배정 이력까지 관리하면 조직 변경 관리를 시스템이 떠안게 되는데 그것은 인사 영역이다. 승인 기록에 승인자가 남으므로 부서장 스냅샷도 별도로 두지 않는다. **`owner_name` 이관 판단(20번)은 여전히 미해결.** 상세는 `docs/adr/ADR-0031-role-authorization-model.md`.

20. **`owner_name` 문자열과 `control_owner` 역할 배정의 관계 (미결)** — RCM 담당자 컬럼 `owner_name`은 **문자열**이며 계정과 연결돼 있지 않다. baseline 93건에 담당자 이름이 문자열로 들어 있어(엑셀 원천의 "담당자" 열) **이관 판단이 필요하다.** 선택지: ①`owner_name`을 유지하고 역할 배정을 별개로 둔다(표시용 vs 권한용 분리) ②역할 배정으로 통합하고 `owner_name`은 표시 파생값으로 만든다 ③계정 매칭을 시도하되 매칭 실패분은 문자열로 남긴다. **이름 문자열이라 동명이인·퇴사자·미가입자 처리가 걸린다.** ADR-0031 §5 미해결에도 등록돼 있다 — 역할 배정 구현 착수 전 결정 필요.

21. **평가 회차·워크플로 확정 (ADR-0032, 제안(초안))** — ADR-0031 후속. **증빙 편집 권한이 회차 마감 상태에 달려 있어 회차 개념이 선행 필요**했다. 결정 요지: **회차는 전담부서(`assessor`)가 생성**하고 통제책임자는 만들 수 없다. **대상 통제는 통제의 평가주기 속성으로 자동 선정**되며(회차 생성 시 통제를 하나씩 고르지 않는다) 회차는 주기별로 병렬 운영된다. **설계평가 회차와 운영평가 회차는 독립**이며 서로 참조하지 않는다. **마감은 회차 단위로만** 하고 개별 통제 마감은 없다. **미완 상태에서도 마감할 수 있되 사유를 남긴다.** 증빙은 **통제×회차** 단위에 다중 사용자가 업로드하며 **물리 삭제는 없다.** **보존기간은 최소 5년을 시스템이 강제**(내부회계관리제도 업무지침 — 회계정보 및 관련 문서 5년 보관)하고 **기본값은 영구**, 5년 이상 범위에서 `icfr_manager`가 설정한다. **주기 지원 범위는 주·월·분기·반기·연 5종이며 최소 단위는 주다** — 일 단위는 회차를 매일 생성하고 증빙을 매일 등록해야 해서 실무가 감당할 수 없다. 매일 발생하는 통제(자금 이체 등)는 주 단위 회차에서 표본을 추출해 테스트한다. 상세는 `docs/adr/ADR-0032-assessment-cycle-workflow.md`.

22. **아카이브 기능 (ADR-0032 §5, 착수 시점 미정)** — 장기 보존 데이터를 수정 불가 형태로 별도 공간에 보관하고, 특정 기간의 설계·운영·평가·증빙·감사·보고 일체를 열람하는 기능. **설계·운영·평가·보고 모듈이 모두 구현된 뒤에 가능하므로 별도 ADR로 다룬다.** **본 단계에서 지는 책임은 "그때 아카이브 가능한 형태로 데이터를 쌓는 것"까지다** — 이력 보존, 물리 삭제 금지, 시점 조회 가능. 지금 설계에서 이 세 가지를 어기면 나중에 아카이브를 만들 수 없다.

**역할 저장소 부채 (2026-09-04 등록 — 3-1 STEP 0 실측에서 발견)**
...
**프롬프트 작성 규칙 (2026-09-04 등록)**

26. **커밋 분리 지시는 파일 구조를 전제한다.** 한 모듈에 들어갈 코드를 커밋 3개로 나누라는 지시는 성립하지 않는다. 프롬프트 작성 시 실제 파일 구성을 확인하지 않고 커밋 분리를 지정하면 구현자가 **억지로 파일을 쪼개거나 지시를 어기는 선택을 강요받는다.** 2026-09-04 3-2에서 발생(커밋 3·4·5 → `e2d3d15` 단일 커밋). 회차 CRUD·활동 기록·마감이 전부 `app/api/assessment.py` 한 모듈이라 분리할 수 있는 경계가 없었다 — 파일을 쪼개면 라우터가 갈라져 경로 구성이 흩어진다. 현 상태 유지로 판단(마스터 승인).

27. **`seed_baseline --reset` 은 더 이상 안전하지 않다 (2026-09-04 발견)** — `role_assignments.target_id`(3-1)·`cycle_targets.control_id`·`assessment_activities.control_id`(3-2)가 baseline id 를 **FK 없이** 가리킨다(정체성 id 규칙상 FK 를 걸 수 없다 — baseline/instance 두 테이블에 걸쳐 있어 걸면 add 통제가 배제된다). 재시딩하면 baseline id 가 바뀌어 **배정·회차 대상·활동 기록이 조용히 끊어진다.** 현재 배정 0건이라 영향은 없으나, 배정이 생긴 뒤에는 금지 경로다. 엑셀 원천이 바뀌어 백필이 필요하면 재시딩이 아니라 UPDATE 마이그레이션으로 처리할 것.

28. **push 절차 규칙 — 마이그레이션 포함 커밋은 Claude Code 가 push 하지 않는다 (2026-09-04 확립)** — 3-2 프롬프트 §8 에 "마이그레이션이 포함되므로 마스터가 백업 후 push한다"라고 적었으나 **Claude Code 가 push 를 실행했다.** 사용자가 "푸시하자"라고 했고, 실행자는 백업 여부를 확인할 방법이 없었다. 결과적으로는 배포 지연으로 백업을 확보했으나 **우연이었다.**

    **교훈: 실행자가 확인할 수 없는 조건은 게이트가 되지 못한다.** 절차를 지시문에 적는 것과 실행 흐름에 심는 것은 다르다. "백업 후 push" 는 백업을 확인할 수 없는 실행자에게는 아무 제약이 아니며, 그 문구는 읽히기는 해도 걸리지는 않는다.

    조치 — `CLAUDE.md` §8.3 절차에 **판정 가능한 단계**로 넣었다. push 전 `git diff --name-only origin/main..HEAD | grep alembic/versions/` 를 실행하고, 결과가 있으면 **사용자가 "푸시하자"라고 해도 push 하지 않고 보고한다.** 이건 실행자가 스스로 판정할 수 있는 조건이다.

    배경: `deploy.yml` 에 경로 필터가 없고(13.8) 컨테이너 기동이 `alembic upgrade head` 로 시작하므로 **push = 운영 마이그레이션 실행**이다. 되돌리려면 백업이 있어야 한다(ADR-0030 §5).

**증빙(3-3) 관련 (2026-09-04 등록)**

29. **`evidence_files` 기존 4건은 시드·테스트 잔재이며 `cycle_id` NULL 인 레거시 증빙으로 남는다.** MinIO 실물은 `test2.pdf` 12바이트 1건뿐. 경로에 `tenant_id` 가 없으나 조회가 DB 레코드의 `minio_key` 를 쓰므로 신규 규칙과 충돌하지 않는다. 정리 시점은 미정이며 실데이터 삭제이므로 마스터가 직접 실행한다. **경로 이동(백필)은 하지 않기로 확정** — 옮길 실물이 1건뿐이고, 파일 이동은 복사 후 삭제라 중간 실패 시 복구가 필요하며, **마이그레이션이 파일을 지우는 것은 §2.4 원칙과 어긋난다.**

30. **`evidence_links` 1건의 `linked_entity_id` 가 레거시 `controls` 테이블만 가리킨다** — `baseline_controls`·`control_instances` 어디에도 없다. 2-A-2 이후 resolver 경로로 조회되지 않는 통제를 참조하는 상태다. 시드 데이터(2026-05-22)라 실무 영향은 없으나, **`evidence_links` 가 통제를 어느 테이블 기준으로 참조해야 하는지가 정의되지 않았다.** 신규 증빙은 통제×회차 직접 부착이므로 이 테이블의 향후 용도 판단 필요. 3-3 에서 손대지 않았다.

31. **증빙 업로드가 `file.read()` 로 전체를 메모리에 적재한다.** ADR-0032 §2.7의 "크기 상한 없음"을 실현하려면 **스트리밍 전환이 선행되어야 하며**, `sha256` 계산과 `length` 확보 방식을 함께 바꿔야 한다. 현재는 상한 50MB 를 유지하며, 하드코딩 대신 정책 설정으로 옮긴다. 디스크 압박 시 서버 볼륨 증설로 대응한다.

32. **운영 디스크 기준선 (2026-09-04, 3-3 착수 시점)** — 증빙 운영 시작 후 이 값과 대조해 증가 추이를 판단한다.

    | 대상 | 값 |
    |---|---|
    | `/data` | 49G 중 사용 75M, 여유 47G (1%) |
...
**EUC·IUC (2026-09-18 등록)**

43. **ADR-0033 EUC·IUC 관리 모델 확정 (제안(초안))** — 사이드바의 EUC·IUC 두 메뉴가 빈 화면인 이유는 두 개념의 관계와 데이터 구조가 정의되지 않았기 때문이었다. 결정 요지: **EUC 는 ERP 밖의 도구**(현업이 만든 엑셀·액세스·매크로 — IT부서 통제를 받지 않는다)이며 ERP 안의 자동 계산은 ITGC 소관이다. **IUC 가 상위 개념이고 EUC 는 그 한 종류다** — IUC 는 통제에 쓰는 *정보*의 완전성·정확성, EUC 는 그 정보를 만드는 *파일*의 관리. **데이터는 통제 중심 하나, 화면은 EUC·IUC 둘** — 같은 대상을 두 테이블로 나누지 않는다(역할 저장소 3곳, risk-categories 읽기/쓰기 분리에서 이미 겪었다). IUC 중요성과 EUC 위험평가의 중요성 축은 같은 판단이므로 **한 곳에서만 입력받는다.** **위험평가는 복잡도 × 중요성 2축이며 이번엔 입력 기반**이다 — 파일을 업로드하지 않고 목록만 관리하므로 자동 판정할 대상이 없다(자동화는 후속, 그때도 입력값을 덮어쓰지 않고 근거로 제시). 위험도 moderate 이상이면 통제 식별 대상이며 **임계값은 설정으로 둔다.** **EUC 통제 4종(백업·접근·변경·모니터링)은 RCM 밖에서 관리한다** — RCM 은 재무보고 프로세스 통제이고 EUC 통제는 도구 관리 통제다. **EUC 평가는 회차에 묶이되 RCM 과 독립**이다 — ADR-0032 §2.2 가 설계·운영평가 회차를 독립으로 둔 것과 같은 방식으로 회차 `kind` 에 추가하며 새 개념을 만들지 않는다. 정보 Type 값 목록은 상수 한 곳에 둔다(13.9-40 교훈). 상세는 `docs/adr/ADR-0033-euc-iuc-model.md`.

    **원천 양식 반입 (2026-09-22)** — `backend/seeds/6__EUC_inventory.xlsx`, `backend/seeds/7__IUC_inventory.xlsx`. **구현 시 1차 출처다.** 평가주기 열이 원천 엑셀에 있었는데 파서가 읽지 않던 일(2026-09-04)이 반복되지 않게 양식을 레포에 두고 참조한다. 파일명은 ADR 표기에 맞춰 공백을 `_` 로 바꿨다(ADR 은 무수정).

    **8건 일치 대조 (로컬 DB, 2026-09-22)** — EUC Inventory 8행·IUC(IPE Template) 8행의 통제활동번호와 `baseline_controls` 의 `ipe_relevant='Y'` 8건이 **정확히 같다**: `FA-050-10-10`·`FR-050-10-20`·`HR-020-10-20`·`SD-020-20-30`·`SD-030-40-10`·`TR-020-20-10`·`TR-020-30-10`·`TR-030-20-10`.

    **구현 전에 알아둘 양식 내용** (원천에서 읽은 사실, 판단은 구현 시):
    - EUC `위험평가` 8건 **전부 Low**, IUC `Information 중요성` 8건 **전부 L** — 두 값이 같은 판단이라는 §2.2 와 맞는다. 다만 **§2.4 임계값(moderate 이상) 기준이면 현재 통제 식별 대상은 0건**이다.
    - `정보 Type` 8건 전부 `EUC information`, `시스템/어플리케이션` 전부 `엑셀파일`, `ITGC In-Scope` 전부 `N` — §2.7(지금은 한 값만 지원) 과 맞는다.
    - EUC `파일변경주기` 8건 전부 `Annually`. 값 범례는 `E/D/W/M/Q/S/A/건별` 이다 — **RCM `frequency`(O/D/W/M/Q/A)와 값 집합이 다르다**(E·S·건별이 더 있다). **결정(2026-09-22): EUC 전용 상수를 따로 둔다.** RCM `FREQUENCY_VALUES` 를 재사용하면 E·S·건별이 선택지와 집계에서 조용히 빠진다 — 대시보드 0건 미표시(13.9-40)와 같은 함정이다. 이름이 비슷해도 축이 다르면 상수를 나눈다(수행주기·평가주기를 나눈 것과 같은 이유).
    - `저장소` 8건 전부 `추후 사이냅소프트 작성` — 아직 비어 있는 값이다.
    - `식별된 EUC Control Test(예시)` 시트는 앞 4열이 `#REF!` 인 **예시 양식**이다. 4종 통제(백업·접근·변경·모니터링)의 위험·통제 상세 문구는 여기서 가져올 수 있다.
    - IUC 시트 `max_row` 는 254 지만 데이터는 8행이다 — 서식만 늘어난 빈 행이다. 파서는 `No` 열 기준으로 끊을 것.

**다음 작업 (2026-09-22 확정)**

44. **다음 작업은 EUC·IUC 구현이다(ADR-0033).** → **5-1 인벤토리 로컬 구현 완료(2026-09-22, 13.9-45). 다음은 5-2(EUC 통제 4종 + 평가 회차 `kind` EUC 추가).** **4-3 역할 배정 화면은 뒤로 미뤘다.** 원천 양식(`backend/seeds/6__EUC_inventory.xlsx`·`7__IUC_inventory.xlsx`)과 13.9-43 의 양식 메모를 먼저 볼 것. 착수 전 짚을 것 — ①양식상 위험평가가 8건 전부 Low 라 §2.4 임계값 기준 통제 식별 대상이 0건이다 ②파일변경주기는 EUC 전용 상수 ③RCM `euc_description`·`related_systems`·`ipe_relevant` 와의 관계(ADR-0033 §3).

45. **5-1 EUC·IUC 인벤토리 구현 (로컬 검증 완료, 마이그레이션 `f8a9b0c1d2e3` push 대기)** — ADR-0033 정정본대로 `euc_files`(독립 대상)와 `information_items`(통제에 딸림)를 두고 `information_items.euc_file_id` 로 N:1 연결했다. **확정한 결정** — ①값 상수는 모델 한 곳(`models/euc.py`·`models/iuc.py`)에 두고 선택지는 `GET /api/euc/meta` 가 준다. 위험 등급 코드는 소문자(`low`/`moderate`/`high` — 원천 수식이 `"moderate"` 로 비교), 파일변경주기는 **EUC 전용** `E/D/W/M/Q/S/A/adhoc`(원천 실값 `Annually` 는 별칭으로 `A`). ②**복잡도는 시드에서 NULL(미평가)** — 원천에 매크로 여부만 있고 「매크로·링크·모델」은 링크·모델도 포함해 추정할 수 없다. 원천 `위험평가`(Low)는 `source_risk_rating` 에 참고값으로 보존하며, **산출 등급과 둘 다 있는데 다르면 `source_mismatch` 로 검토 신호를 띄운다.** 그래서 대시보드는 「식별 대상 0 · **미평가 8**」로 나온다 — 원천이 Low 로 적은 것과 우리가 평가하지 않은 것은 다른 상태다. ③권한 — 정보 항목은 `icfr_manager` + 그 통제의 `control_owner`·`assessor`(통제 단위, `resolve_roles_for_control`), EUC 파일은 생성 `icfr_manager`, 수정·삭제는 `icfr_manager` + 참조 통제의 `control_owner`. `require_write` 를 쓰지 않았다(13.9-35·41 재발 방지). 화면 게이트는 서버가 행마다 주는 `can_edit`·`writable_control_ids` 를 따른다. ④**통제 FK 없음** — 통제 id 가 baseline/instance 두 테이블에 걸친 정체성 id 라 성립하지 않는다(프롬프트 지시 정정). 존재 검증은 핸들러가 `resolve_controls` 로 한다. ⚠️ **13.9-27 위험 승계** — 이제 `seed_baseline --reset` 은 정보 항목 연결까지 조용히 끊는다. 파일 참조는 복합 FK `(euc_file_id, tenant_id)` 로 테넌트 격리(**postgres 에서 교차 테넌트 참조가 FK 위반으로 거부됨을 확인**). ⑤파일 중요성·위험 등급·통제 식별은 **저장하지 않고 조회 시 산출**(`services/euc_risk.py`, 매트릭스는 초기값임을 주석에 명시). ⑥**통제 제외 시 effective 제외**(2026-09-22 추가 결정) — `resolve_controls` 에 없는 통제의 항목은 목록·집계·파일 중요성에서 빠지고 복원하면 돌아온다. 참조 통제가 전부 제외된 파일은 「참조 통제 0건」으로 남는다. ⑦유니크는 처음부터 부분 유니크(`WHERE NOT is_deleted`). ⑧임계값 정책 `euc_identification_threshold`(기본 moderate)는 저장 시 값 검증(422).

    **양식 파싱에서 걸린 것** — IUC 헤더 두 열의 첫 줄이 같다(`시스템/↵어플리케이션` 과 `시스템/↵어플리케이션 In-Scope…`). 첫 줄을 키로 쓰면 한 열이 조용히 덮어써진다 → 헤더 전체를 키로 쓰고, 앞부분 검색이 두 열 이상에 걸리면 중단하게 했다(`Report Logic` 과 `Report Logic 관련 control` 도 같은 쌍). EUC 시트 일부 셀이 외부 워크북 참조 수식이라 `data_only=True`(캐시값)로 읽는다.

46. **RCM 3개 필드와 EUC·IUC 의 관계 (판단만, RCM 미수정 — 후속 과제)** — 실측(로컬, 2026-09-22):

    | 필드 | 실측 | 판단 |
    |---|---|---|
    | `euc_description` | 8건 모두 EUC 파일명과 **글자까지 같음**, 나머지 85건 `N/A` | **중복.** 정보 항목 → EUC 파일명에서 파생 가능 |
    | `ipe_relevant` | `Y` 8건 = `euc_description` 값이 있는 8건 = 정보 항목이 있는 8건 | 정보 항목 존재 여부로 **대체 가능.** 두 곳에서 따로 관리되면 어긋난다(ADR-0033 §5) |
    | `related_systems` | 7건 `더존`, 1건 `N/A`. IUC 「시스템」은 8건 모두 `엑셀파일` | **용도가 다르다.** RCM 쪽은 통제가 쓰는 ERP, IUC 쪽은 정보를 만드는 도구 |

    **후속 과제 등록**: `euc_description`·`ipe_relevant` 를 정보 항목에서 파생하도록 전환(RCM 화면·엑셀 업로드·대시보드 IPE 묶음 영향). 전환 전까지는 **두 곳이 어긋날 수 있다** — 정보 항목을 추가·삭제해도 RCM `ipe_relevant` 는 그대로다.

**테스트 환경 (2026-09-22 등록)**

47. **테스트 환경 격리 — 백엔드 pytest 는 동시에 두 개를 돌릴 수 없는 구조다 (미수정, 기록)** — `tests/conftest.py` 가 DB 를 **고정 경로 `sqlite:///./test.db`** 하나로 쓰고, 세션 fixture 가 시작 시 `create_all`·끝날 때 **`drop_all`** 을 한다. 그래서 pytest 프로세스 두 개가 같은 작업 디렉터리에서 겹치면 **먼저 끝난 쪽이 다른 쪽의 테이블을 지운다.** 증상: 코드 변경과 무관하게 수십~수백 건이 한꺼번에 실패·에러가 난다(`no such table`, 로그인 실패 연쇄 등). 실사례 — 5-1 에서 전체 실행 중에 단일 테스트(`-k tenant`)를 하나 더 띄워 **186 failed / 36 errors** 가 났고, 순차 재실행에서 297 passed × 2 였다. 마스터 보고로는 그 전에도 같은 충돌이 여러 번 있었다.

    **지금 규칙: 백엔드 pytest 는 한 번에 하나만 돌린다.** 전체 실행이 백그라운드로 도는 동안 단일 테스트를 따로 띄우지 않는다. **대량 실패가 나면 코드 결함으로 단정하기 전에 동시 실행 여부부터 확인한다.** "2회 반복 실행" 검증도 반드시 순차로 한다.

    근본 조치 후보(별도 지시 대기): 프로세스마다 다른 DB 파일(예: `tmp_path_factory` 또는 PID 접미사), 또는 인메모리 SQLite + 공유 커넥션. CI 는 잡 하나에서 순차 실행이라 영향이 없다 — **로컬 개발에서만 나는 문제**다.

48. **✅ 2026-09-22 해소 — 증빙 연결(`evidence_links`) 쓰기 권한 구멍 — 외부감사인이 증빙 연결을 만들고 지울 수 있다 (2026-09-22 6-1 STEP 0 발견)** — **우선순위: 6-1 완료 직후, 매뉴얼 패널(13.9-8) 작업 전에 별건으로 막는다.** 등록만 해두는 항목이 아니다. `api/evidence.py` 의 `POST /api/evidence/links`(`create_link`)·`DELETE /api/evidence/links/{link_id}`(`delete_link`) 가 가드 없이 `CurrentUser` 만 받는다 — 로그인한 사람이면 **역할과 무관하게** 누구나 통제와 증빙 파일의 연결을 만들고 끊을 수 있다. `external_auditor` 도 포함된다. **감사 증거 훼손 경로다** — 감사인이 제출된 증빙의 연결을 끊으면 통제에서 증빙이 사라지고, 누가 끊었는지는 소프트 삭제 컬럼 외에 이력이 없다. 조치 방향(별건 지시 대기): 쓰기 둘을 파일 업로드와 같은 기준(최소 `external_auditor` 거부, 통제 단위 권한이면 `icfr_manager` + 그 통제의 `control_owner`·`assessor`)으로 가드하고, 거부 테스트(외부감사인·무역할 사용자 403)를 붙인다. 조회(`GET /links`)는 인증 유지. 13.9-35·41 과 같은 계열 — 쓰기 엔드포인트가 `CurrentUser` 만 받는지 **증빙 모듈 전체를 함께 점검**할 것.

    **해소 (2026-09-22, 마스터 결정 3건)** — STEP 0 실측: FE 호출 0 · 백엔드 소비 0 · **운영 `evidence_links` 0건**(마스터 확인)이라 권한을 좁혀도 영향받는 데이터가 없다. 점검 중 **같은 계열 구멍 2건을 더 찾아 함께 막았다.** ①**연결 생성·삭제 `icfr_manager` 전용** — 쓰는 곳이 없는 경로에 통제책임자·평가자 판정을 따로 만들지 않는다. 실제 용도(13.9-49 등)가 생기면 업로드 판정과 함께 정한다. 대상 종류 허용 목록(`models/evidence.py` `LINK_TARGET_TYPES` = `control`, 밖이면 422)·통제 존재 확인(`resolve_controls`, 404)·자기 회사·미삭제 파일 확인(404 — `file_id` FK 가 복합 FK 가 아니라 조회로 검증)·마감 회차 잠금(파일이 회차 × 통제에 붙어 있으면 업로드와 같은 판정 — 지금은 관리자만 오므로 통과, 권한을 넓힐 때 걸린다). ②**삭제는 소프트 삭제 + `deleted_by`(사용자 id)·`deleted_at`**, 생성은 `created_by`. 핸들러가 직접 채운다(13.9-51). 사유 칸은 별건. ③**`PATCH /files/{id}` — 로그인만 확인했고 `minio_key` 까지 바꿀 수 있었다**(증빙 레코드가 다른 파일을 가리키게 만드는 경로). `minio_key` 를 수정 스키마에서 빼고 알 수 없는 필드는 422(`extra="forbid"`), 파일명 수정은 업로드와 같은 판정. ④**레거시 증빙(회차 없음) 수정·삭제는 `icfr_manager` 만** — 전에는 `require_write` 만 적용돼 외부감사인이 아닌 누구나 지울 수 있었다. 증빙 모듈 쓰기 엔드포인트 5개(파일 생성·수정·삭제, 연결 생성·삭제)를 전부 점검했고 파일 생성은 이미 막혀 있었다. 테스트 `tests/test_evidence_guard.py` 7건. 계약 문서 `docs/api/evidence-contract.md` §3.1 갱신. 마이그레이션 없음.

49. **증빙 첨부 대상 확장 — 통제 외 대상(스코핑 검토 기록 등)에 파일을 붙일 수 없다 (후속, 2026-09-22 등록)** — 증빙 모듈은 사이클×통제에 묶여 있다(`evidence_links` 의 대상이 통제). 6-1 스코핑의 외부감사인 검토 증빙은 그래서 **텍스트 참조**(`scopings.review_evidence_ref` — 문서명·보관 위치)로 두었다(마스터 승인 7). 파일 첨부는 증빙 모듈이 다형 대상을 받도록 확장하는 별건이며, 13.9-48 권한 정리 뒤에 한다(구멍 난 채로 대상을 넓히지 않는다). 코드 주석(`models/scoping.py`·`schemas/scoping.py`)이 이 번호를 참조한다.

50. **6-1 스코핑 코어 구현 (로컬 검증 완료, 마이그레이션 `a9b0c1d2e3f4` push 대기)** — ADR-0034(제안 초안, 커밋 `89d1142`)대로. 원천 `backend/seeds/4__내부회계관리제도_Scoping.xlsx`. **확정한 결정** — ①**재무제표 종류 규칙**(승인 5): 원천 각주대로 BS·PL 만 양적 판정, 주석(NOTE)·현금흐름(CF)은 「해당 없음」(`na`) — 「미평가」(입력 부족)와 다른 상태로 화면·집계에서 구분. 결론 = 양적 OR 질적, 양적이 해당 없음이면 질적으로. ②**템플릿은 전역**(승인 6, `IdentityBase`, code+version, `ICFR_STD v1`) — 계정 192·질적 평가값·판단 근거·수동 판정 6건까지 담는다. 회계연도 생성 시 **복사**하고, 필드 단위 출처(`scoping_field_origins`, 테넌트 소유)로 「템플릿」 배지를 붙인다. 같은 값 재저장은 편집이 아니다. **출처 테이블은 대상이 다형이라 대상 FK 가 없다 — 핸들러가 검증**(모델 주석에 명시). 상태는 enum(draft→review→confirmed, 재오픈은 사유 필수). 확정은 배지가 남아도 막지 않고 개수를 이력·확정 기록에 남긴다. ③**계산은 저장하지 않는다**(ADR-0029 §2.2) — 조회 시 `services/scoping_calc.py` 가 산출. Excel ROUND 를 `Decimal` ROUND_HALF_UP 으로 재현(float 금지): 산출액 ROUND(0) → 전반 ROUND(-3) → 수행 ROUND(0). 원천 캐시값 재현 확인(4,818,843,993×5% → 240,942,200 → 240,942,000 → 168,659,400). 확정 시 스냅샷(정책·결론)을 값으로 굳힌다 — 이후 정책을 바꿔도 확정 결론·대시보드 집계는 그대로. ④질적 기준은 테넌트 정책 `scoping_qual_threshold`(기본 2)·`scoping_qual_comparison`(기본 `ge` 이상) — 저장 시 값 검증(422). **원천 문구 "2(Medium)을 초과할 경우"는 실제 수식(`>=`)과 어긋나 "이상일 경우"로 고쳐 적재**(4개 시트). `gt` 로 바꾸면 정확히 16개 계정 결론이 뒤집힌다(테스트 고정). ⑤쓰기 전부 `require_icfr_manager`(`require_write` 금지), 확정 상태 쓰기 409. ⑥검토 증빙은 텍스트 참조(13.9-49). **회사 식별자 치환**(승인 6 조건) — 스캔 대상 사이냅소프트·Synap·주식회사 사이냅소프트·약칭·감사법인/담당자명. 적재 범위(가이던스·질적 요소·판단 원칙·Notes·계정·판단 근거)에서 **회사명은 0건**(표지·3.1 시트의 감사인 이메일은 적재하지 않음). 치환은 「당사→회사」 1건(Notes 「1. 당사의 개요」→「1. 회사의 개요」). 적재기가 금지어(`@`·회사명·감사법인명)를 만나면 중단한다. ⚠️ **회사 고유 내용 1건 — 설정근거(D68) 「회사의 주요영업이 해외 자회사를 통해…」** 는 이름은 없지만 특정 회사 사정이라 템플릿 문구로 부적절할 수 있다(**2026-09-22 마스터 결정: 그대로 둔다** — 템플릿 배지와 확정 시 「템플릿 그대로인 필드 N개」 경고로 드러난다). **원천 불일치 처리**(승인) — 조정 항목은 부호 붙여 합산(FY2022 SUM 기준, FY2021 은 차감식이었음), 「재고자산」 그룹 행(결론 빈칸·평가값 전부 H)은 계정으로 적재하지 않음, 매출액 가이드 「0.0~5%」는 오기 의심(실사용 0.5%) — **마스터 확인 전까지 범위 경고를 걸지 않는다**(2026-09-22 재확인 — 확인 대기, 경고 판정에 계속 쓰지 않는다). 그 외 Notes 시트 `#REF!`·FY2019 머리글은 적재하지 않음. GITC·프로세스 연결 대조는 6-2. **postgres 격리 확인**(트랜잭션 후 롤백): 교차 테넌트 참조 FK 거부(`scoping_accounts`·`scoping_field_origins`), 자기 테넌트 참조 통과, 같은 연도 테넌트별 공존, 같은 테넌트 중복 연도 부분 유니크 거부, 롤백 후 잔존 0.

    **계정 192 건의 구성 (2026-09-22 push 전 확인)** — BS 58 · PL 45 · 주석 38 · 현금흐름 51. STEP 0 보고는 **191(주석 37)** 이었고 차이 1건은 **`2.3 유의한 주석` 31행 `13. 사용권자산`** 이다. 금액 칸이 `=5005866000+1227635000`(숫자끼리 더한 수식)이라 STEP 0 집계가 「수식이면 소계」로 세어 빠뜨렸다. 평가값 10개·질적 결론 수식·판단 근거가 모두 있는 **실제 주석**이고 주석 번호 1~38 이 빠짐없이 이어진다 — **적재가 맞고 STEP 0 집계가 틀렸다.** 적재기는 같은 시트 셀을 참조하는 수식만 소계로 본다(`_is_subtotal`). 제외하기로 한 BS B34 `(2) 재 고 자 산` 그룹 행은 하위 계정이 없는 행이며 **들어오지 않았다**(이름·그룹명 모두 0건). 두 사실을 테스트로 고정(`test_template_account_counts_by_statement`·`test_subtotal_rule_needs_cell_reference`).

    **운영 적용 절차(마스터 실행)**: 백업 → push → 배포 확인(`alembic upgrade head` 가 `a9b0c1d2e3f4` 까지) → `docker exec icfr-backend python -m seeds.seed_scoping_template`(테넌트 인자 없음 — 전역 템플릿, 재실행하면 「이미 있음」) → 확인 `docker exec icfr-postgres sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "select code, version, (select count(*) from scoping_template_texts t where t.template_id = s.id) texts, (select count(*) from scoping_template_accounts a where a.template_id = s.id) accounts from scoping_templates s;"'` → 기대값 `ICFR_STD | 1 | 20 | 192`. 종류별 확인이 필요하면 `select statement_type, count(*) from scoping_template_accounts group by 1 order by 1;` → `BS 58 · CF 51 · NOTE 38 · PL 45`. 회계연도 스코핑은 화면에서 내부회계관리자가 만든다.

51. **감사 컬럼 `created_by`·`updated_by`·`deleted_by` 가 시스템 전체에서 비어 있다 (✅ 2026-09-22 해소 — 로컬 커밋, push 대기(마이그레이션 없음). 등록: 2026-09-22 13.9-48 중 확인)** — `AuditedBase`(`TimestampMixin`·`SoftDeleteMixin`) 의 세 컬럼을 **채우는 코드가 어디에도 없다.** 로컬 postgres 실측: 컬럼이 있는 **52개 테이블 전부**, 데이터가 있는 32개 테이블의 **채워진 값 0건**(삭제 표시된 행 27건도 `deleted_by` 0). **왜 `tenant_id` 는 채워지고 사용자 id 는 안 채워지나** — `tenant_id` 는 `get_current_user` 가 요청마다 ContextVar(`tenant_context._active_tenant_id`)에 활성 테넌트를 넣고 `before_flush` 리스너가 신규 `TenantMixin` 객체에 찍는다(ADR-0025). **사용자 id 에 대한 ContextVar 도, 리스너도 만든 적이 없다** — 격리(누출 방지)는 ADR-0025 로 자동화했지만 감사 흔적은 자동화 대상에 넣지 않았다. 그래서 "누가"가 필요한 곳은 **도메인 컬럼을 따로 두어 왔다**: 증빙 `uploaded_by_id`·`deleted_by_id`(계정 FK), 스코핑 `confirmed_by_id`·상태 이력 `actor_id`, 역할 배정 이력 등. 이번 13.9-48 은 `evidence_links` 에 한해 핸들러가 직접 `created_by`·`deleted_by` 를 채웠다. 조치 후보(별도 지시 대기): 요청 단위 사용자 ContextVar + `before_flush` 에서 신규 `created_by`·변경 `updated_by`·소프트 삭제 `deleted_by` 를 찍기. 컬럼이 문자열이라 계정 FK 가 아니다 — 형식(사용자 id 문자열)을 정하고, 시스템 맥락(시드·마이그레이션)은 비워 둘지 정해야 한다. 기존 행은 채울 근거가 없어 비운 채 둔다.

    **✅ 해소 (2026-09-22, ADR-0036, `prompts/ICFR_backend_13-9-51_20260922.md`)** — tenant 와 같은 구조로 자동 기록한다. ①**행위자**: `core/audit_context.py` ContextVar. `get_current_user` 가 `str(user.id)` 를 설정하고, 사용자 없는 쓰기는 `system_actor("system:<출처>")`(형식 `^system:[a-z][a-z0-9-]*$`)로 **명시해야만** 기록된다. 출처 7종 — `bootstrap`·`seed-users`·`seed-baseline`·`seed-euc-iuc`·`seed-scoping-template`·`migration-rcm-baseline`·`test`. ②**기록 규칙**(before_flush, 별도 리스너): insert → `created_by`(이미 있으면 유지)·`updated_by`, update → `updated_by`, `is_deleted` false→true → `deleted_by`·`deleted_at`, 복구 → 둘 다 비움. 대상은 믹스인 판별로 **매핑 52개 전부**(IdentityBase 7개 포함). ③**fail-closed**: 행위자가 없으면 insert·update·hard delete 모두 `MissingActorError` 가 나고 flush 가 실패한다. ④**bulk UPDATE/DELETE 차단**(`AuditBypassError`, 도입 시점 사용처 0건). `text()` raw SQL·alembic `op.*` 는 막지 못한다(한계, ADR §2.5). ⑤**테스트 행위자는 `session.info` 로 직접 삽입 세션에만 붙인다** — 테스트 스레드 ContextVar 가 TestClient 요청 안으로 전파되는 것을 실측했다. ContextVar 로 걸면 API 경로 누락이 가려진다. API 세션·운영 `SessionLocal` 에 info 가 없다는 것을 테스트로 고정했다. ⑥`api/evidence.py` 링크의 `created_by`·`deleted_by`·`deleted_at` 수동 대입을 제거했다(`evidence_files.deleted_at_ts` 는 도메인 컬럼이라 유지). STEP 0 에서 "deleted_at 수동 대입 2곳"이라 보고했으나 실제 기반 컬럼 대입은 1곳이었다(나머지 1곳은 응답 스키마 인자). ⑦기존 NULL 행 backfill 없음. 스키마 변경 없음(`String(255)`). 테스트 `tests/test_audit_columns.py` 70건(52개 모델 순회 + 시스템 행위자·형식·fail-closed·info 가드·bulk·bootstrap·API·다른 tenant). 로컬 컨테이너 실 API 확인은 **미확인**(backend 재시작 반복).

52. **제도 운영 흐름과 스코핑 이후 단계 (기록만, 2026-09-22 6-1b §6 — 구현 전)** — 마스터 확정 흐름: **회사가 기준 설정 → 감사인 검토·감사 → 확정 → 확정 기준으로 운영 → 감사·감사위원회에 내부 보고·승인 → 지적 사항은 개선과제로 → 이듬해 개선.** 6-1·6-1b 는 앞의 셋(설정·검토·확정)만 다룬다. 이어받을 모듈 — ①**감사위원회 내부 보고·승인 → Report** ②**스코핑 지적 사항 → 개선계획**: 감사인 검토 의견(`scopings.review_opinion`)이나 확정 후 발견된 스코핑 지적을 개선과제로 넘기는 연결. 지금은 연결 필드가 없다 ③**이듬해 이월 → 스코핑**: 새 연도를 직전 연도 **확정본**에서 시작하는 기능(ADR-0034 §5). 지금은 새 연도가 항상 템플릿에서 시작하고, 중요성 기준만 테넌트 정책을 기본값으로 쓴다. 이월 시 배지를 어떻게 둘지(전년 확정값 = 이미 검토된 값인가, 다시 확인할 값인가)가 설계 쟁점이다. 근거: ADR-0034 §6 정정 이력(§2.2).

53. **6-1b 스코핑 보완 구현 (로컬 검증 완료, 마이그레이션 `b1c2d3e4f5a6` push 대기)** — `prompts/ICFR_6-1b_scoping_refine_20260922.md`, ADR-0034 §6 정정 이력. **확정한 결정(마스터 2026-09-22)** — ①**검토 확인**: 출처 상태에 `confirmed` 추가(`template`/`confirmed`/`edited`). 단위는 계정 한 줄·중요성 기준 영역·**문구 한 항목**(문구 19개에도 둔다 — 없으면 전부 확인해도 경고가 19로 남는다). `POST /api/scoping/{id}/confirm {scope, target_id, undo}`. 확인자·시각은 새 칸 `confirmed_by_id`·`confirmed_at`(감사 컬럼 `updated_by` 를 빌리지 않는다 — 13.9-51). `confirmed` 를 고치면 `edited`, 확인 취소 허용, 확정 상태에서는 409. **확정 경고(`badge_count`·`confirm_badge_count`)는 `template` 만 센다.** ②**중요성 기준 저장 위치 B안** — 비율 가이드 범위(`scoping_benchmarks.guide_low/high`)·설정율 범위(`scopings.smt_guide_low/high`)·질적 기준값·비교 방식(`scopings.qual_threshold/qual_comparison`)·기준 연도(`scopings.base_fiscal_year`)를 **회계연도 스코핑에** 둔다. 테넌트 정책 `scoping_qual_*` 는 새 연도 생성 시 기본값으로만 쓴다(정책을 바꿔도 이미 만든 스코핑 판정은 불변 — 테스트 고정). 범위 기본값은 템플릿 `default_criteria`(원천 Note 1·2), **매출액은 비움** — 비어 있으면 경고 없음, 회사가 범위를 넣으면 경고. 기준 칸도 기본값에서 왔으므로 템플릿 배지가 붙는다(범위는 두 칸이 배지 하나). 판정 기준은 비울 수 없다(422), 하한 > 상한 422. ③**확정 스냅샷에 기준값 전부** — 벤치마크 6종 기준값·조정 후 기준값·비율·산출액·가이드 범위, 조정 항목, 설정율 범위, 질적 기준, 기준 연도, 계정별 기준 금액·전년 금액·질적 평균. 6-1 키(`policy`)는 유지. ④**기준 연도·두 해 비교** — 기본 회계연도 − 1, 라벨 "기준 금액 (FY{기준 연도} 결산)"·"전년 금액 (FY{기준 연도 − 1})", 증감률 `(기준 − 전년)/|전년|`(전년 0·없음이면 표시 안 함). **양적 판정은 기준 금액만.** `prior_amount` 칸은 6-1 에 이미 있었다. ⑤**가독성** — 판단 근거·수동 판정 사유를 내용 높이만큼 늘어나는 입력칸으로, 열 최소 폭 22rem, 긴 글은 "전체 보기"로 펼친다. 수동 판정 사유를 줄 안에 보인다. **마이그레이션 기존 행 처리(마스터 규칙)** — 템플릿 `default_criteria` 채움, 작성 중·검토 중 스코핑은 새 기준 칸을 템플릿 기본값(질적 기준은 그 테넌트 정책 → 없으면 2 이상)으로 채우고 템플릿 배지를 붙임, 기준 연도 = 회계연도 − 1. **확정 스코핑은 채우지 않고 건너뛰며 건수를 출력한다** — 확정 기록을 사후에 바꾸지 않는다. 마이그레이션을 실패시키지 않은 이유: push = 운영 마이그레이션이라 멈추면 배포가 깨진다. 대신 **push 전에 마스터가 확정 스코핑 유무를 조회**한다(아래). 확정본의 빈 칸은 조회 시 기본값으로 대신 보이고 확정 판단은 스냅샷이 보존한다. **로컬 검증** — 작성 중 1건·확정 1건을 넣고 upgrade: 작성 중은 기준 채움 + 배지 8(설정율 범위·질적 2·벤치마크 범위 5, 매출액 제외), 확정은 전부 NULL. downgrade 로 출처 행·칸 원복, 재 upgrade 정상, 검증 행 삭제(로컬 스코핑 0건). 테스트 scoping 37건(신규 10 — 줄 확인·확인 후 수정·취소·**전부 확인하면 0**·권한/404/422·기준 연도·전년 금액·범위 회사 설정·매출액 무경고·정책은 새 연도 기본값·**기준 변경 후 스냅샷 보존**). 브라우저 확인은 못 했다(로컬 백엔드 컨테이너 재시작 반복 — 13.9-24).

    **운영 적용 절차(마스터 실행)** — ①**push 전 확인**: `docker exec icfr-postgres sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "select fiscal_year, status from scopings where not is_deleted order by 1;"'` — `confirmed` 가 있으면 그 연도는 새 기준 칸이 비워진 채 남는다(의도). 괜찮으면 진행 ②백업 ③push ④배포 확인(`alembic upgrade head` → `b1c2d3e4f5a6`, 로그에 `[6-1b] 기준 칸 채움: N건 / 확정이라 건너뜀: M건`) ⑤확인 `docker exec icfr-postgres sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "select fiscal_year, status, base_fiscal_year, smt_guide_low, smt_guide_high, qual_threshold, qual_comparison from scopings where not is_deleted;" -c "select status, count(*) from scoping_field_origins where not is_deleted group by 1;"'` — 작성 중 2026 은 `2025 | 0.5000 | 0.7500 | 2.00 | ge`, 배지 `template` 은 적용 전 개수 + 8(설정율 범위 1·질적 기준 2·벤치마크 범위 5 — 2026 을 만든 직후 그대로면 1,889 + 8 = 1,897). 시드 재실행은 필요 없다.
...
- **2026-09-22 / TrustBuilder + Claude** — **6-1b 스코핑 보완** (`prompts/ICFR_6-1b_scoping_refine_20260922.md`, ADR-0034 §6 정정 이력). 운영 2026 스코핑의 "템플릿 그대로 1,889개" 경고가 정보를 주지 못한 문제 → 배지 상태 `confirmed`(검토 확인, 계정 줄·중요성 기준·문구 단위, 확인자·시각, 취소 가능) 추가, 확정 경고는 `template` 만. 중요성 기준(비율 가이드 범위·설정율 범위·질적 기준·기준 연도)을 회계연도 스코핑으로 옮기고 테넌트 정책은 새 연도 기본값으로만(B안). 매출액 범위 기본값 비움. 확정 스냅샷에 기준값 전부. 기준 금액 = 직전 연도 결산(라벨 정정) + 전년 금액·증감률. 판단 근거 가독성. 신규 13.9-52(운영 흐름·이월·지적→개선과제 기록)·53(구현 기록·운영 절차). 테스트 scoping 37건, 백엔드 전체 순차 2회, FE tsc·vitest 42·build. 마이그레이션 `b1c2d3e4f5a6` — 확정 스코핑은 채우지 않음, push 는 마스터(백업 후).
- **2026-09-22 / TrustBuilder + Claude** — **CLAUDE.md §10 작업 세션 규칙** — 새 작업은 새 세션·`/model opusplan`, STEP 0 은 plan 모드, 테스트는 실패만 보이게, 순차 2회는 마이그레이션 작업에만. Regina 동일.

- **2026-09-22 / TrustBuilder + Claude** — **13.9-48 증빙 쓰기 권한 구멍 수정** (마스터 결정 3건, 마이그레이션 없음). 증빙 연결 생성·삭제를 `icfr_manager` 전용으로(허용 목록 422·통제 404·파일 404·마감 회차 판정), 연결 삭제는 소프트 삭제 + 누가·언제. 점검 중 발견한 2건도 함께 — `PATCH /files/{id}` 가 로그인만 확인하고 `minio_key` 까지 바꿀 수 있던 것(경로 수정 불가 + 업로드와 같은 판정), 레거시 증빙 삭제가 `require_write` 만이던 것(`icfr_manager` 만). 신규 13.9-51(감사 컬럼 시스템 전체 미기록 — 별건). 테스트 `test_evidence_guard.py` 7건, 백엔드 전체 순차 2회. 계약 문서 §3.1.

- **2026-09-22 / TrustBuilder + Claude** — **6-1 스코핑 코어** (`prompts/ICFR_6-1_scoping_core_20260922.md`, ADR-0034 제안 초안). 전역 템플릿 `ICFR_STD v1`(문구 20·계정 192·수동 판정 6)을 원천 엑셀에서 적재하고 회계연도마다 복사, 필드 단위 「템플릿」 배지. 중요성·양적·질적·결론은 저장하지 않고 조회 시 산출(Excel ROUND 재현), 확정 시 스냅샷. 주석·현금흐름은 양적 「해당 없음」, 입력 부족은 「미평가」로 구분. 질적 기준은 테넌트 정책(기본 2 이상) — 원천 문구 「초과」를 수식대로 「이상」으로 정정. 쓰기 `require_icfr_manager`. 회사 식별자 치환 1건(당사→회사), 회사 고유 설정근거 1건은 그대로 두기로 결정(배지·확정 경고로 드러남). 신규 13.9-48(🔴 증빙 연결 권한 구멍 — 다음 작업)·49(비통제 대상 첨부)·50(구현 기록). 테스트: scoping 29건(계정 구성 192 = BS 58·PL 45·주석 38·현금흐름 51 고정 포함), 백엔드 전체 순차 2회, FE tsc·vitest 42·build, postgres 격리 5건. 마이그레이션 `a9b0c1d2e3f4` — push 는 마스터(백업 후).

- **2026-09-22 / TrustBuilder + Claude** — **13.9 백로그 8번 재결정 — 용어 해설 → 모든 메뉴·기능·항목의 매뉴얼 패널** (문서만). ①범위: 용어뿐 아니라 메뉴·기능·항목 전부. ②배치: 08-24 결정(본문 위 겹침)을 **본문을 밀어내는 방식**으로 변경, 패널 폭 조절 가능, **좁은 화면은 겹침으로 전환**(RCM 표 보호). ③구조: 문구를 화면 코드에 넣지 않고 **키로 따로 저장**, 용어 사전과 같은 저장소. ④콘텐츠: ADR 문구를 쓰지 않고 사용자 말로, 규정은 출처·조번호·요지·기준일. ⑤순서: **6-1 완료 후·6-2 착수 전에 패널 틀과 키 규칙**, 6-2 부터 새 화면은 처음부터 키, 기존 화면은 키 일괄·문구 점진. 기존 서술은 정정 이력으로 남겼다.

- **2026-09-22 / TrustBuilder + Claude** — **5-1 EUC·IUC 인벤토리** (`prompts/ICFR_5-1_euc_iuc_inventory_20260918.md`, ADR-0033 정정). ①**ADR-0033 정정 먼저** — EUC 파일을 독립 대상으로(엑셀 하나를 여러 통제가 공유하는 경우를 통제 중심 구조는 표현하지 못한다), 복잡도는 파일·중요성은 정보 항목에 입력하고 파일 중요성·위험 등급·통제 식별은 산출, 통제 제외 시 effective 제외. 초안은 덮어쓰지 않고 정정 이력으로 덧붙였다. ②**구조** `RCM 통제 1:N information_items N:1 euc_files` — 통제 FK 는 없음(정체성 id), 파일 참조는 복합 FK. ③**산출값 비저장**(ADR-0029 §2.2). ④**시드는 원천 그대로** — 복잡도 NULL, 원천 위험평가는 참고값으로 따로 보존 → 대시보드 「식별 0 · 미평가 8」. ⑤권한 명시(통제 단위), `require_write` 미사용. ⑥화면 — EUC(파일 중심, 참조 통제·산출 등급·원천 비교), IUC(통제 중심), 대시보드 EUC·IUC 카드. ⑦검증 — 공유 파일 1건 유지, 최고값 산출·L→H 연동, 매트릭스 9칸, 임계값 정책 전환, 제외/복원, 권한(남의 통제 403·external_auditor 전부 403), 재생성, **postgres 교차 테넌트 FK 거부**. BE 26건 추가(매트릭스 9칸 포함). 상세 13.9-45·46. 마이그레이션 `f8a9b0c1d2e3` 포함이라 **push 대기**.

- **2026-09-18 / TrustBuilder + Claude** — **ADR-0033 EUC·IUC 관리 모델 작성** (`docs/adr/ADR-0033-euc-iuc-model.md`, 상태 제안(초안), 문서만). 근거 자료는 실무 양식 `6__EUC_inventory.xlsx`·`7__IUC_inventory.xlsx`. **두 양식이 같은 8건**(유지보수매출·대손충당금·미수수익·금융상품평가·미지급이자·연차충당부채·리스·이연법인세)을 다루며 **대시보드 `ipe_relevant` 관련 8건과 일치함을 확인**했다 — EUC·IUC 가 별개 목록이 아니라 통제의 두 측면이라는 판단의 근거다. 요지는 13.9-43. ADR-0028~0032 와 같이 개별 파일이며 §10 요약은 두지 않는다(`CLAUDE.md`·`README.md` 분리 기준에 0033 추가). 초안은 무수정으로 반입했다.
...
- **2026-09-04 / Regina + Claude** — **상위 3계층(Process/SubProcess/Risk) CRUD UI** (`prompts/rcm_상위3계층_crud_ui.md`, 커밋 `d2c4002`, **로컬 커밋까지만 — push 대기**). 13.9-14(신규화면 필요 확정) 착수. 프롬프트가 근거로 지정한 `docs/api/rcm-hierarchy-contract.md`가 **저장소 어느 브랜치에도 커밋된 적이 없어** — 대신 실제 백엔드 구현(`backend/app/api/rcm.py`, `backend/app/schemas/rcm.py`, 2-A-4-3)을 직접 읽어 계약을 확인하고 진행했다(스키마·409 규칙·PATCH 무시 필드 전부 코드에서 재확인). ①**미결 결정 2건(13.15) 해소**: UI 위치는 프롬프트가 이미 "RCM 모듈 내부 신규 탭"으로 확정해 왔음. envelope required 전환은 2026-09-02 작업으로 이미 완료돼 있어 이번 작업의 전제조건이었다. ②**RCM 페이지에 "계층 관리" 탭 신규**(`UsersPage.tsx`의 사용자/역할 토글 패턴을 3-way로 확장 — Tabs 컴포넌트가 아니라 Button 토글, 페이지 레벨에서 그 패턴이 이미 쓰이고 있었으므로 그대로 미러링). Process/SubProcess/Risk 각 목록 + 생성(상위 드롭다운 전용, 자유입력 금지) + 편집(code·상위참조 disabled — 서버가 조용히 무시하는 필드를 애초에 못 건드리게) + 삭제. ③**신규 파일**: `hierarchyApi.ts`/`useHierarchy.ts`(controlsApi.ts/useControls.ts 그대로 미러링 — 프로세스 목록만 기존 `fetchProcesses`를 재사용해 `ControlSearchBar`/`BasicInfoTab`의 드롭다운과 캐시 공유, sub-process/risk는 "전체 목록" queryKey가 필요해 `subProcessesAll`/`risksAll` 신설), `errors.ts`(`extractHierarchyErrorMessage` — 3계층 공통 단일 409 핸들러, 서버 `detail`을 그대로 노출하고 충돌 종류를 문자열로 재해석하지 않음. Control 쪽 `ControlFormDialog`의 기존 핸들러는 하드코딩 메시지라 손대지 않고 그대로 둠 — 범위 밖), `permissions.ts`(`canEditHierarchy()` — 편집/삭제 버튼 노출 단일 seam, 지금은 무조건 `true`, ADR-0031 역할 판단 미확정), `components/hierarchy/` 6개(FormDialog×3은 `UserRoleFormDialog.tsx`의 단순 폼 패턴을 미러링 — Control의 4탭짜리 `ControlFormDialog`보다 필드가 훨씬 적어 더 가까운 전례였다. Table×3, `HierarchyDeleteConfirmDialog`는 Control 전용 `DeleteConfirmDialog`를 envelope 기반 exclude/soft_delete 문구로 일반화). ④**기존 파일 확장**: `controlsAdapter.ts`에 단일 어댑터 `toProcess`/`toSubProcess`/`toRisk` 추가(기존 `toProcessItems` 등은 이를 재사용하도록 리팩터), `ProcessItem`에 백엔드가 이미 보내던 `description` 필드 보강(그동안 목록 어댑터가 버리고 있었음), `queryKeys.ts`에 무효화 전용 `subProcessesAll`/`risksAll` 추가. ⑤**테스트**: `jsdom`이 설치돼 있지 않아(`vite.config.ts`는 `environment: 'jsdom'`을 지정하지만 devDependencies에 없음 — 기존 환경 갭, 이번 작업 범위 밖) 컴포넌트 렌더 테스트는 불가능함을 먼저 확인. `sourceEnvelope.test.ts`와 동일하게 `@vitest-environment node`로 어댑터·에러 핸들러·권한 seam만 순수 로직 테스트 10건 추가(`hierarchy.test.ts`). ⑥**검증**: `tsc --noEmit` 에러 0, `npm run build` 통과, `npx vitest run` 27/27(기존 17 + 신규 10). **브라우저 실 API 왕복·운영 미확인** — push 전 사용자 확인 대기. 다음: 브라우저 검증 후 push, upload-excel 파서 분리(13.9-10-b)는 이 작업과 무관하게 별도 진행 가능.

- **2026-09-04 / TrustBuilder + Claude** — **3-1 부서·역할 배정 구현 + 계약 문서 + 이해상충 판정 정정** (`prompts/ICFR_3-1_org_roles_20260903.md`, 커밋 8건). ①**STEP 0 실측이 ADR 전제를 뒤집었다** — ADR-0031 §3 은 "`users` | 역할은 사용자 속성이 아님 — 변경 없음"이라고만 적었으나 **역할 저장소가 이미 3개 존재**했다(`users.role` / `user_tenant_access.role` / `user_roles`). 마스터 판단으로 **테넌트 단위 역할 5종은 기존 `user_roles` 재사용**(AuditedBase 라 격리 기존재·다중 역할 가능·CRUD/FE 배선 기존재), `users.role` 은 시스템 관리 권한으로 존치. **§3.1 저장소 결정·§3.2 경계**(`users.role` 과 `user_roles` 는 서로 참조하지 않는다)를 ADR 에 명문화했다. ②**구현**: `departments`·`user_departments`·`role_assignments`·`conflict_acknowledgements`·`tenant_policies` 5테이블 + `/api/org/*` 15개 엔드포인트. **주 소속은 사용자당 1건을 DB 부분 유니크로 강제**(앱 검증만 두면 한 경로만 빠뜨려도 뚫린다), 테넌트 격리는 ADR-0030 방식 복합 FK — postgres 에서 교차 참조 2방향 거부 실증. **`role_assignments.target_id` 에는 FK 를 걸지 않았다** — 대상이 baseline 유래면 `baseline_*.id`, add 면 `*_instances.id` 라 참조 테이블이 하나로 정해지지 않는다(정체성 id 규칙). FK 를 걸면 add 항목 배정이 막히므로 핸들러가 resolver 결과와 대조한다. ③**배정 해석은 `control_resolver` 와 공용화하지 않았다** — 그쪽은 두 테이블 병합, 이쪽은 한 테이블 안 `scope` 우선순위라 매개변수가 맞지 않는다. ④**`owner_name` 이관하지 않음 확정** — 실측 93건 중 복수 인물 18(19%)·팀 이름 4(4%)이고 `유헌종
노정희` 와 `노정희
유헌종` 이 별개 문자열로 공존해 매핑 불가. 참고 정보로만 응답에 싣는다. ⑤**계약 문서 `docs/api/org-contract.md` 신설** — 모든 응답 예시가 실왕복 원문. RCM 규약과 다른 4건(배정에 PATCH 없음·해석 응답 구조가 달라 RCM 어댑터 재사용 불가·정책 목록 페이지네이션 없음·낙관적 잠금 부재) 명시. ⑥**이해상충 판정 정정(13.9-25)** — 계약 문서를 쓰다 `control_owner = dept_approver` 가 **배정 하나만으로 충돌로 잡히는 것**을 발견. 팀장이 통제책임자인 통제는 전부 이 형태라 실무에서 흔하다. **겸직이 아니라 부서승인 단계가 성립하지 않는 상황**으로 판단해 판정 대상에서 빼고 `dept_approval_skipped` 상태로 전환했다. `source` 에 `"skipped"` 를 넣지 않은 이유 — `source` 는 출처라는 단일 의미이고 스킵은 상태라 섞으면 RCM envelope 개념과 어긋나며, 스킵일 때 유도된 부서 책임자가 누구인지 표현할 자리가 없어진다. ⑦**부수 수정**: 정책 토글 테스트가 테넌트 전역 상태를 남겨 뒤 테스트를 오염시키던 것을 발견 — 켠 테스트가 되돌리도록 고쳤다. ⑧13.9-23(`user_tenant_access.role` 판정 코드 0건)·13.9-24(`user_roles` 기존 3행 `Reviewer` 의미 불명확) 부채 등록. 기존 3행은 **운영 데이터 변경이라 건드리지 않았다.** **로컬 검증만 완료 — 운영 미적용.**

- **2026-09-03 / TrustBuilder + Claude** — **ADR-0032 평가 회차·워크플로 작성** (`docs/adr/ADR-0032-assessment-cycle-workflow.md`, **상태 제안(초안)**. 문서만, 코드 변경 0건). ADR-0031 후속이며 **증빙 편집 권한이 회차 마감 상태에 달려 있어 회차 개념이 선행 필요**했다. ①**회차 생성 주체와 대상 선정**: 회차는 `assessor`(전담부서)가 생성하고 통제책임자는 만들 수 없다. **대상 통제는 통제의 평가주기 속성으로 자동 선정**되며 — 회차 생성 시 통제를 하나씩 고르지 않는다 — 회차는 주기별로 병렬 운영된다. ②**설계평가와 운영평가 회차는 독립**이며 서로 참조하지 않는다. ③**마감은 회차 단위로만** 하고, **미완 상태에서도 마감할 수 있되 사유를 남긴다.** ④**증빙은 통제×회차 단위**에 다중 사용자가 업로드하며 **물리 삭제가 없다.** ⑤**보존기간은 최소 5년을 시스템이 강제**한다(근거: 내부회계관리제도 업무지침 — 회계정보 및 관련 문서 5년 보관). 기본값은 영구이며 5년 이상 범위에서 `icfr_manager`가 설정한다. ⑥**주기 지원 범위를 좁혔다 — 주·월·분기·반기·연 5종, 최소 단위는 주.** 초안은 일 단위까지 지원했으나 **회차를 매일 생성하고 증빙을 매일 등록해야 해서 실무가 감당할 수 없다**고 판단했다. 시스템이 지원해도 사용되지 않으면 유지 비용만 남는다. 자금 이체처럼 매일 발생하는 통제는 주 단위 회차에서 해당 기간의 **표본을 추출해 테스트**하는 방식으로 대응하며, 표본 선정 방식은 Test 모듈 설계 시 다룬다(§5 미해결로 이관). 이 결정으로 "회차 자동 생성(스케줄러) 여부 — 일 주기 통제는 수동 생성이 비현실적" 미해결 항목은 해소돼 삭제했다. ⑦**ADR-0031 미해결 2건 해소**: §5의 "평가 회차별 역할 변동"과 §2.8의 "당시 부서장이 누구였는가"를 **배정 이력을 관리하지 않고 수행 기록에 수행자를 남기는 방식**으로 처리한다(§2.8). 감사에서 묻는 것은 "1분기 평가를 누가 했는가"이고 그 답은 평가 기록에 있다 — 배정 이력까지 관리하면 조직 변경 관리를 시스템이 떠안게 되는데 그것은 인사 영역이다. ⑧13.9 에 21번(ADR-0032 확정)·22번(아카이브 기능 — 설계·운영·평가·보고 모듈 완성 후 별도 ADR, **본 단계 책임은 아카이브 가능한 형태로 데이터를 쌓는 것까지**) 등록, 19번에 미해결 2건 해소 반영. `CLAUDE.md`·`README.md` ADR 분리 기준에 0032 추가. **잔여 미해결**: `owner_name` 이관 판단(13.9-20), 표본 추출 방식, 미비점 관리, MinIO 테넌트 격리 방식.

- **2026-09-03 / TrustBuilder + Claude** — **ADR-0031 역할·권한 모델 작성** (`docs/adr/ADR-0031-role-authorization-model.md`, **상태 제안(초안)**. 문서만, 코드 변경 0건). **발단은 증빙 관리 설계 중 통제별 담당자 배정 체계 부재를 발견한 것이다** — 증빙 편집 권한이 "해당 통제의 평가자"에 달려 있는데 그 배정 체계가 없었다. 역할이 정해지지 않으면 "누가 무엇을 할 수 있는가"를 쓸 수 없으므로 선행 결정으로 작성했다. ①**용어 혼동 확인**: 조사 결과 **`reviewer` 가 통제 실행 층과 평가 층에서 다르게 쓰인다.** 통제 실행 층(preparer/reviewer/approver)은 통제 활동 그 자체 — 대사표 작성→팀장 검토 같은 것으로 **본 시스템의 관리 대상이 아니라 통제 문서에 기술될 내용**이다. 글로벌 문헌의 `creator/approver/reviewer` RBAC 논의는 대부분 이쪽(ERP 등)을 말한다. 본 시스템은 평가 층을 다루므로 그 용어를 그대로 가져오면 안 된다 — **평가 층 용어인 `control_owner`/`assessor` 를 채택**했다. ②**역할은 사람이 아니라 통제 단위로 배정**(§2.2) — 관리부서 4명인 회사에서 회계 담당이 회계 통제의 책임자이면서 인사 통제의 평가자가 되는 상호 배정이 불가피하고 국내 실무에서 허용된다. 역할을 사람 속성으로 두면 이 구조를 표현할 수 없다. ③**이해상충은 막지 않고 경고+사유 기록**(§2.4) — 막으면 중소기업이 시스템을 못 쓰고, 조용히 허용하면 감사에서 지적된다. 사유 기록이 보완통제 증적이 된다. ④**배정 단위는 프로세스 기본값 + 통제별 예외**(§2.3) — 통제 93건 × 역할 4종을 개별 배정하면 372회 조작이다. baseline/overlay와 같은 구조(ADR-0029 §2.1·§2.2 원칙 적용). ⑤**'ITGC의 시스템'과 '본 시스템'은 다르다**(§2.1.1) — ITGC의 통제 대상은 회사의 ERP·인사·자금 시스템이고, 본 시스템은 그 통제를 문서화·평가 기록하는 도구다. 혼동하면 `sys_admin` 이 ITGC 통제책임자로 자동 배정되거나, IT부서 담당자에게 `sys_admin` 을 줘서 제도 운영 데이터를 직접 수정 가능하게 만드는(감사 지적 대상) 오류가 난다. ⑥**부서 도입**(§2.8) — 회사 조직도의 **스냅샷**이며 원본은 인사시스템. 발령·겸직·개편 로직은 구현하지 않고, 관리 주체는 인사담당자가 아니라 내부회계전담부서 담당자다. 계층은 `parent_id` 컬럼으로 개념만 보존하고 **평면으로 구현**한다 — 나중에 컬럼을 추가하려면 마이그레이션이 또 필요하고, **스키마 변경은 횟수 자체가 위험**이다(ADR-0030 경험). 인사시스템 연동용 `external_code` 도 선반영. ⑦13.9 에 19번(ADR-0031 확정·후속 0032 예정)·20번(`owner_name` 문자열과 역할 배정의 관계 — baseline 93건에 담당자 이름이 문자열로 있어 이관 판단 필요) 등록. `CLAUDE.md`·`README.md` ADR 분리 기준에 0031 추가. **후속 ADR-0032**(평가 회차·워크플로)에서 다룰 것: 평가 회차별 역할 변동, 부서 스냅샷 시점("당시 부서장이 누구였는가"), 역할 배정 이력 수준, 평가 회차 전체에 대한 최종 승인.

- **2026-09-03 / TrustBuilder + Claude** — **13.9-17 통제 code 중복 검증 누락 수정** (커밋 2건 `f3b0f29`(fix) + 문서. **로컬 커밋까지만 — 응답 형태 변경(201→409)이라 Regina 공유 후 마스터가 push**). 전날 계약 정리(`80adabc`)에서 등록한 결함을 닫았다. ①**수정**: `create_control`(`rcm.py:692`)에 `_assert_code_available` 호출 추가. **헬퍼와 호출 형태는 상위 3계층(`rcm.py:255/310/363`)을 그대로 따랐고 변형하지 않았다** — label 만 `"통제"`. ②**409 응답 원문 대조**(TestClient 실왕복): 통제 `"통제 코드 'X' 는 표준(baseline)에 이미 있습니다"` / 프로세스 `"프로세스 코드 'X' 는 표준(baseline)에 이미 있습니다"` — **구조 `{라벨} 코드 '{code}' 는 {사유}` 로 완전히 동일하고 라벨만 갈린다.** instance 충돌 문구(`"이미 사용 중입니다"`)도 동일. **Regina 는 통제 폼과 3계층 폼의 409 처리를 통합할 수 있다**(충돌 종류 구분이 필요하면 `"표준"`/`"사용 중"` 포함 여부로 판별 가능하나 문자열 매칭이라 백엔드 문구 변경에 취약하다는 점은 전달). ③**검증 5개 전부 통과**: baseline 충돌 409 / instance 충돌 409 / 정상 code 201(회귀 없음) / 상위 3계층 동작 불변(`test_rcm_hierarchy_api.py` 기존 케이스 포함 31 passed) / 전체 `pytest` **176 passed·1 skipped·1 xfailed·0 failed**(173→176, 신규 3건), `ruff` All checks passed. ④**문서 동시 갱신**: `docs/api/rcm-hierarchy-contract.md` — §5 표 Control 열을 실제 응답으로 교체, §6-① 을 해소 이력으로 전환(왜 이 검증이 필요한지 = baseline 과 instance 가 서로 다른 테이블이라 DB 제약이 걸치지 않는다는 점은 남겼다), 상단 미해결 경고 제거, 스냅샷 기준 커밋을 `af4c247` → `f3b0f29` 로 갱신. 13.9-18(낙관적 잠금 부재)은 **미해결로 남아 있다** — 4계층 공통이라 이번 범위가 아니었다.

- **2026-09-03 / TrustBuilder + Claude** — **상위 3계층 CRUD API 계약 정리 + 결함 2건 등록** (조사 전용 — **백엔드 코드 변경 0건**, 문서만). Regina 의 프론트 배선 전 계약 확인 요청. 추정 없이 맞추려는 것이라 `schemas/rcm.py`·`api/rcm.py`·`control_resolver.py` 에서 **읽은 것만** 정리해 `docs/api/rcm-hierarchy-contract.md` 로 남겼다(스냅샷 문서 — 상단에 기준 커밋 `af4c247` 명시, API 변경 시 갱신). ①**계약 동일 확인**: envelope 3필드(`source`/`baseline_id`/`is_overridden`)는 4계층 flat 동일하며 `_resolve_layer` 공통 함수가 생성한다. 상위 3계층은 **응답을 내는 4개 경로(목록·상세·POST·PATCH)가 전부 resolver 를 거치므로** envelope 가 항상 채워진다 — FE 의 required 전환(Regina, `8fe0ba4`)과 정합. id 체계·`exclude_unset`·DELETE 서버 분기·목록 봉투·정렬 전부 동일. **`code` 와 상위 참조는 4계층 모두 PATCH 대상이 아니다**(정체성/구조) — 그 UI 를 만들면 서버가 조용히 무시한다. ②**차이 1건 발견 → 13.9-17 등록**: `_assert_code_available` 이 상위 3계층 3곳에만 있고 `create_control` 에는 없다. **통제는 baseline 과 code 가 겹쳐도 검증 없이 201** 이고 resolver 결과에 같은 code 가 둘 나온다. 2-A-4-3(`cad62a9`)에서 헬퍼를 신설하며 2-A-4-1 에 만들어진 control 로 소급하지 않은 누락으로 판단 — 의도된 차이로 보이지 않는다. **수정하지 않고 별건으로 등록**했다(지시 범위가 확인까지였다). ③**4계층 공통 미비 1건 → 13.9-18 등록**: 낙관적 잠금이 계약에 없다. `row_version` 컬럼은 있으나 스키마·핸들러 언급 0건 — 동시 편집 시 마지막 쓰기가 이긴다. 사용자 2명인 현재는 발현이 낮으나 고객사 다중 사용자 환경에서는 감사 증적 문제가 된다. ④**공통 동작 1건 기록**: 존재하지 않는 상위 id 를 보내도 `_resolve_*_parent` 가 `(None, None)` 을 반환해 **201 + 상위 null** 로 생성된다(4계층 공통, 3계층 배선의 신규 위험은 아님). ⑤`CLAUDE.md` §3·`README.md` 의 `docs/api` "(예정)" 표기 정정 — 디렉토리에 실제 문서가 생겼고, 자동 생성 문서가 스펙의 단일 진실 공급원이라는 점과 `openapi.yaml` export 미착수는 유지. **13.9-17 수정 시 계약 문서 §5·§6-① 을 함께 갱신해야 한다** — 지금은 그 결함을 현재 동작으로 기술하고 있어, 고쳐지면 문서가 틀리게 된다.
- **2026-09-02 / Regina + Claude** — **RCM 상위 3계층 envelope required 전환 (조회 계층)** (`prompts/ICFR-PROMPT-envelope-required-transition.md`, ADR-0027). 2026-08-13 전환(control 계층 한정, `ICFR-PROMPT-2A3-1-envelope-required.md`)이 상위 3계층을 제외한 이유는 당시 `/processes`·`/sub-processes`·`/risks`가 resolver 미배선이었기 때문(13.7)인데, 2-A-4-3(2026-08-24)이 그 미배선을 해소했으므로 이번엔 FE 타입만 따라잡으면 되는 작업이었다. 착수 전 **사전 조사**(`ICFR-PROBE-mutation-envelope-wiring.md`, read-only)로 control mutation 배선 패턴부터 확인 — `isBaseline`/`resolveDeleteSemantics` 헬퍼는 mutation 엔드포인트 분기에 관여하지 않고 순수 UI 문구(안내 배너·삭제 확인 다이얼로그) 전용이며, 실제 쓰기는 소스 무관 단일 POST/PATCH/DELETE라는 기존 계약(13.4 항목6)을 재확인했다. ①원격 동기화(`git fetch`) 결과 13개 커밋 차 확인 — ADR-0030(baseline 테넌트 소유권) 운영 적용·어서션 junction/risk-categories 전환 등, 이번 작업 대상(process/sub_process/risk 조회 타입)과 겹치지 않아 곧장 `pull`. ②`types.ts`의 `ProcessItem`/`SubProcessItem`/`RiskItem.envelope` optional(`?`)→required. ③`controlsAdapter.ts`의 `toProcessItems`/`toSubProcessItems`/`toRiskItems`를 `buildOptionalSourceEnvelope`→`buildSourceEnvelope`(control과 동일, 부재 시 명시적 실패)로 교체. ④**프롬프트 사전조건에 없던 실측**: `tsc --noEmit`이 `dto.ts`의 `ProcessItemDto`/`SubProcessItemDto`/`RiskItemDto`(wire 타입) 3종에서 타입 에러 3건을 냈다 — 이 DTO들도 여전히 `source?`/`baseline_id?`/`is_overridden?` optional이었기 때문. 조회 경로 안쪽(같은 3계층 wire 타입)이라 판단해 `ControlDto`와 동일하게 required로 함께 전환. `buildOptionalSourceEnvelope`는 정의·단위테스트만 남고 실사용처 0건(제거는 범위 밖). ⑤**검증**: `tsc --noEmit` 에러 0. 라이브 API 3계층 재검증(`curl` + JWT) — `source`/`baseline_id`/`is_overridden` 3필드 전부 항상 존재 확인(런타임 throw 위험 없음). `sourceEnvelope.test.ts` 17건 통과. **브라우저 검증**: 로컬에 Playwright/chromium-cli 등 브라우저 자동화 도구가 없어 `npm install --no-save`로 Playwright를 임시 설치(패키지·잠금파일 변경 없음, 검증 후 즉시 제거)해 headless 로그인→RCM 진입 스크립트로 확인 — "전체 93건 중 1–20" 정상 렌더, 통제코드 아래 "기준" SourceBadge 노출, 콘솔 에러 0건. 13.7에 해소 기록 추가. 다음: 상위 3계층 mutation 훅 자체가 아직 부재(PROBE 조사 4번 항목) — 신규 작성은 이 작업 범위 밖.
...
- **2026-09-01 / TrustBuilder + Claude** — **13.9-10-a upload-excel 멀티헤더 0건 수정** (`prompts/ICFR_excel_fix_20260901.md`, 커밋 3건. **로컬 커밋까지만 — 응답 형태 변경이 포함되어 Regina 공유 후 push**). 같은 날 조사(`9ecb514`)로 특정한 원인을 고쳤다. ①**`refactor` `7c30290`** — `seeds/seed_baseline.py` 에만 있던 `_find_data_start` 를 `services/excel_parser.find_data_start_row` 로 이동·공용화. 배치 근거: `ws` 와 컬럼 인덱스만 쓰는 순수 함수이고 `find_rcm_sheet` 가 돌려준 `header_row`·`mapping` 을 그대로 이어받는 후속 단계라 호출 순서상 같은 자리다(`api/rcm.py` 에 두면 seed→api 의존이 하나 늘고, 파서 분리 후속에서 또 옮겨야 한다). 실패 알림이 계층마다 다르므로(seed 는 중단, API 는 응답) 함수는 `None` 만 돌려주고 seed 호출부가 같은 메시지로 `SystemExit` 을 유지 — **seed 동작 불변**을 `test_seed_baseline.py` 8건으로 증명. ②**`fix` `ffdea99`** — 호출부가 `_parse_rcm_sheet(ws, header_row, …)` 로 넘기던 것을 `(data_start - 1)` 로 보정. **엔드포인트(TestClient, `mode=preview`) 실측 0건 → 93건**(조사에서 0건이 재현되던 바로 그 경로). 단일 헤더 양식은 `data_start = header_row + 1` 이라 동작 불변. 함께 **0건을 성공으로 반환하지 않도록** 막았다 — 데이터 시작행 미발견/유효 통제 0건 두 지점 모두 `422 + status: "no_data_rows"`. 형태는 기존 `header_not_found` 규약에 맞췄다(이 건만 다른 규약이면 FE 가 따로 분기해야 한다). 조사에서 드러난 **"에러도 경고도 없이 200"** 이 실제 피해였다 — 93건짜리 RCM 을 올리고도 성공 표시가 뜬 채 아무것도 안 들어간다. ③**`test` `e78c8f4`** — `tests/test_rcm_excel_multiheader.py` 6건. 픽스처는 **합성 + 실파일 둘 다**: 합성은 구조(1~2행 메타·3~4행 공백·5행 밴드·6행 주헤더·7행 2차 헤더·8행 데이터)를 코드로 고정해 원천 파일이 교체돼도 계약을 지키고, 실파일은 합성이 실제와 어긋날 가능성을 막는다(둘 중 하나만으로는 부족하다). 실파일 테스트는 건수를 하드코딩하지 않고 **seed 파서 결과와 대조**한다 — 두 경로가 같은 파일을 다르게 읽던 것이 결함의 본질이므로 검증도 "두 경로가 일치하는가"로 둔다. **로컬 `ruff` All checks passed / `pytest` 167 passed·1 xfailed·0 failed.** 잔여 xfail `test_excel_upload_commit` 은 **10-b(쓰기 경로 overlay 미전환) 소속이라 이번 수정으로 해소되지 않는다**(조사에서 확인한 대로). `EXCEL_UPLOAD_LOCKED` 해제도 10-b 이후 판단 — 파싱은 고쳐졌으나 커밋 경로가 아직 레거시 `controls` 에 쓴다. 다음: 13.9-10-b.
...
- **2026-08-20 / Regina + Claude** — **문서 정리 3·4·1 + 프롬프트 3종 커밋** (`prompts/ICFR-13.5-doc-cleanup-3-4-1.md`, 커밋 `79a2618`·`0ab6613`). 앞서 진행한 문서 중복 스캔(`ICFR-13.5-doc-dup-scan.md`, read-only 조사, 별도 커밋 없음)에서 나온 높은 심각도 항목 중 코드 무영향 3건을 정리. ①**ADR SSOT 결손 해소**: `ClaudeICFR.md` §10에 ADR-0024(git push 2-tier 승인 원칙)·ADR-0025(Baseline+Overlay+멀티테넌시 플랫폼 근간)·ADR-0027(RCM Baseline/Overlay 2단계 설계)을 각 4~5줄로 요약 추가 — 원문(`prompts/ICFR_adr_0024/0025/0027_20260615.md`)에서 발췌, §14·본문의 기존 인용과 번호·제목 일치 확인. ②**미존재 docs 경로 정리**: `Test-Path`로 `docs/adr`·`docs/api`·`docs/erd` 미존재 확인 후 `CLAUDE.md` §3·`README.md` 핵심문서표에 "(예정)" 표기 + 현재 대체 위치(§10/OpenAPI 자동문서/§5) 안내 추가, 실존하는 `docs/diagrams/`도 신규 기재. ③**모듈 수 불일치 정리**: README "9개 핵심 모듈" 나열(ADR-0005/0006 이전 버전, Report·Test 누락)을 ClaudeICFR.md §1.2(SSOT, 11개)로 판단해 나열 자체를 제거하고 참조 문구로 단일화(재발 방지). ④문서 중복 스캔에서 함께 나온 `Regina_ClaudeContext.md`는 이번 커밋에서 **역할 확인만 하고 수정하지 않음**(폐기/유지 판단 보류, 이후 슬림화로 별도 처리 — 위 항목 참조). 프롬프트 파일 3종(`ICFR-13.5-doc-dup-scan.md`·`ICFR-13.6-excel-multiheader-investigate.md`·`ICFR-13.5-doc-cleanup-3-4-1.md`)을 별도 커밋으로 리포에 추가.
- **2026-08-20 / Regina + Claude** — **13.6 upload-excel 다중 헤더 파싱 버그 — FE 조사·BE 핸드오프** (`prompts/ICFR-13.6-excel-multiheader-investigate.md`, read-only 조사 전용). §13.6(2026-08-XX 이전 기록, "부수 발견 — upload-excel 다중 헤더행 미대응")의 파싱 주체가 FE인지 BE인지 확정하기 위한 조사. `frontend/src` 전체에 xlsx/SheetJS 관련 import가 **0건**임을 확인, `features/rcm/api/uploadExcel.ts`의 `previewExcel`/`commitExcel`이 원본 `File`을 가공 없이 `FormData`로 `POST /api/rcm/upload-excel`에 그대로 전달함을 코드로 확인 — **파싱·검증 로직 전부가 백엔드 upload-excel 핸들러 소관**임을 재확인(§13.6 원인 서술과 일치, 신규 사실 없음). FE 쪽 잠금 지점도 함께 확인: `excelUploadLock.ts`의 `EXCEL_UPLOAD_LOCKED=true`(2-A-3-2에서 도입)가 `ControlTable.tsx`의 "Excel 업로드" 버튼 `disabled`만 제어 — 파싱 로직과 무관한 UI 진입 차단용. 스코프 가드("백엔드가 파싱 주체로 확인되면 거기서 멈추고 보고만")에 따라 백엔드 코드(`api/rcm.py` 등)는 열람하지 않음. **결론**: 픽스는 BE(TrustBuilder) 트랙 — 2-A-4-3(13.3·13.5.1·13.6 참조) 범위에서 처리 필요. `EXCEL_UPLOAD_LOCKED`는 BE 픽스 완료 시까지 유지. 코드 변경 없음(read-only).
...
- **2026-08-11 / TrustBuilder + Claude** — **2-A-4-2 통제 다건·목록 전환 + clear-all 제거** (`ICFR_rcm_bulk_2a42_20260807.md`, ADR-0027). ①분기 로직을 `_apply_control_update`/`_apply_control_delete`로 추출 — 단건 PATCH/DELETE와 다건 bulk가 **같은 코드**를 탄다. ADR-0020 범위 내 함수 추출만(클래스·서비스 금지), 커밋하지 않음(트랜잭션 경계는 호출자, 다건은 1회 커밋). ②`bulk-delete`/`bulk-update` → instance 전환, 미해당 id는 `skipped_ids`로 반환(전체를 404로 실패시키지 않음). ③**`bulk-update`의 `exclude_none`→`exclude_unset` 통일** — 이전에는 `False`/`""`가 미전송으로 오인돼 저장 자체가 불가능했다(단건 PATCH와 계약 불일치). ④`GET /controls` → `resolve_controls` 경유 + envelope, search·상세와 정체성 id 일치. **통제 계층 읽기·쓰기 경로 전부 baseline/overlay 통일 완료.** ⑤`clear-all` 제거 — Phase 0 하드삭제 잔재이고 baseline/overlay에선 위험 기능. 재구축은 `seed_baseline.py --reset`이 안전하게 대체(라우트·import·스키마·테스트 4곳, 소스 잔여 0건, FE 미사용). ⑥테스트: bulk·목록 신규 10건 + xfail 마킹 2건 제거(`test_search_text`·`test_control_extended_crud`), `test_excel_upload_commit`은 §4 연쇄로 **xfail 추가**(5→6건, 2-A-4-3에서 해소). ⑦13.5 기존 실패 3건 전부 해소 — 그중 `test_post_creates_add_instance`의 초기 진단(sqlite 오염)이 부정확했음을 정정: 실제로는 `test_rcm_baseline.py`의 add 픽스처가 필수 11필드를 비워 resolver가 NULL을 내보낸 것. **제품 코드 무변경**, 픽스처를 `ControlInstance` 규약("add는 자체 필드 전부 채움")에 맞춰 수정. 전체 pytest **128 passed / 6 xfailed / 0 failed**(2회 재현). 실 Postgres 93건도 새 목록 경로에서 `ControlRead` 변환 성공(source 전량 baseline, is_overridden 0).
- **2026-08-12 / Regina + Claude** — **RCM 프로세스 필터 동적화** (`ICFR-PROMPT-process-filter-dynamic.md`, FE). `ControlSearchBar.tsx`의 하드코딩 `PROCESSES = ['O2C','P2P','R2R','HR','ITG']`(Phase 0 데모값, baseline seed 도입 후 미갱신)가 실제 baseline 8개 프로세스 코드(EL/EX/FA/FR/HR/IT/SD/TR)와 어긋나 HR 제외 전 필터가 0건이 되는 버그를 수정. 사전 확인(구현 전): 필터 값이 이미 `process_code`(code) 기준(name 아님) 확인, `PROCESSES` 상수는 해당 파일 로컬 상수로 재사용처 없음(`export` 없음, 전체 검색 2곳 모두 같은 파일) 확인 후 진행. ①신규 API 작업 없음 — 기존 `fetchProcesses`(`controlsApi.ts`, `BasicInfoTab.tsx`가 이미 사용 중)를 `useQuery`(`queryKeys.rcm.processes(tenantId)`, staleTime 10분)로 연결, 신규 쿼리 키 없어 충돌 없음. ②드롭다운 라벨을 `code — name`(예: `EL — 전사`)으로 교체, 선택값(value)은 `code` 그대로 유지 — 필터 키·동작 무변경. ③옵션 로딩 중에도 "전체"는 항상 노출(빈 배열 매핑이라 자연히 보장). API 계약·백엔드 코드 무변경. `npm run build`(tsc+vite) 에러 0. **브라우저 검증(Regina)**: RCM 관리 화면 필터에서 8개 프로세스 코드 전수 정상 렌더링, 93-기반 기대 분포 중 `EL` 37건/`FA` 7건/`TR` 14건 실측 일치(0건 버그 해소 확인), SourceBadge·mutation lock·콘솔 클린 기존 상태 유지. 검증 중 콘솔에 `/api/auth/me`·`/api/rcm/processes`·`/api/rcm/controls...` 401 3건 일시 발생 — 원인 분석: `fetchProcesses`가 손대지 않은 `fetchControls`와 동일한 공용 `apiClient`(Authorization/X-Tenant-Id 인터셉터 공유)를 사용하고, 이번 세션에서 전혀 건드리지 않은 `/api/auth/me`·기존 controls 호출도 함께 401이 뜬 점으로 보아 신규 코드와 무관한 세션 토큰 만료로 판정 — `axios.ts` refresh 인터셉터로 자동 해소 확인(재로그인 불필요). 13.4 항목1 중 `process_code` 부분(1-b) 완료 처리(assertion/risk_level 부분은 범위 밖, 미완 유지). 알려진 한계 1건 신규 기록(13.7): `GET /api/rcm/processes`는 아직 `resolve_processes` 병합 경로가 아니라 레거시 `processes` 테이블 직접 조회(현재 tenant 1개·instance 전부 0건이라 실질 차이 없음, 향후 processes도 2-B급 전환 시 같이 갱신 필요). 저위험 작업으로 push 자동 승인 대상(프롬프트 명시)이나 커밋 실행 전 사용자 확인 절차(CLAUDE.md 섹션 5/8) 준수.
- **2026-08-11 / Regina + Claude** — **baseline seed 실행 (`--reset`, Regina 로컬)**. 2026-08-06 구축된 `seeds/seed_baseline.py`를 처음으로 실제 실행 — 13.3에 "실행은 마스터 직접 수행"으로 남아있던 방침을 이번 세션에서 변경(사용자 확인). ①실행 전 스크립트 docstring 확인 결과 정식 호출은 `docker compose exec backend python -m seeds.seed_baseline --reset`(모듈 실행) — 최초 시도한 `python seeds/seed_baseline.py`(스크립트 직접 실행)는 Dockerfile에 PYTHONPATH 미설정이라 `from app.api.rcm import ...` 절대 임포트가 깨질 것으로 판단해 실행 전에 교정. ②`--reset`은 실데이터 삭제 경로라 사전 확인 — 로컬 baseline+instance 11테이블이 전부 0행이라 삭제될 실데이터 없음을 사용자와 함께 확인 후 실행. ③실행 결과: `[reset] 삭제 완료 — 총 0행 제거` → 엑셀 재파싱·삽입 → baseline_processes 8/baseline_sub_processes 29/baseline_risks 85/baseline_risk_categories 7/**baseline_controls 93**/baseline_control_assertions 469 전부 엑셀 파싱 수와 `[OK]` 일치, instance 5테이블 0(암묵 adopt), 구 스키마 `controls` 98 등 불변. ④스크립트 자체 출력을 그대로 신뢰하지 않고 `docker compose exec postgres psql -U icfr_user -d icfr_db`로 별도 접속(최초 `-U icfr -d icfr`는 role 없음 에러 → `.env`에서 `POSTGRES_USER=icfr_user`/`POSTGRES_DB=icfr_db` 확인 후 재시도)해 baseline 6테이블 카운트 쿼리 1회 + instance 5테이블·legacy controls 카운트 쿼리 1회로 독립 교차검증 — 스크립트 출력과 완전 일치. **효과**: 13.4 "baseline/instance 데이터 백필 migration 여부" 확인 대기 항목 해소. 항목1을 1-a(DB baseline 원천 검증)/1-b(라이브 API 카운트 검증)로 분리 — **1-a만 완료**(기존 기준값 "95건" 무효화, 엑셀 원천 93건이 정답), **1-b·2·3·4(라이브 API·UI 검증)는 여전히 미완**, 다음 세션 과제로 남음. 코드 변경 없음(seed 스크립트는 08-06에 이미 구축됨, 본 항목은 실행+검증만).
- **2026-08-11 / Regina + Claude** — **흡수 레이어 순수 로직 단위 테스트** (`ICFR-PROMPT-absorption-layer-unit-tests.md`). 2-A-2 재마이그레이션과 무관하게 지금 확정 가능한 범위만 커버 — `frontend/src/features/rcm/api/sourceEnvelope.test.ts` 신규(17건): `buildSourceEnvelope`(flat→nested 조립, id 재사용 확인, DEV throw·프로덕션 폴백), `buildOptionalSourceEnvelope`(레거시 응답 undefined 폴백, 부분 필드 안전 기본값), `isBaseline`/`isTenantAdd`(baseline/tenant/레거시 판정), `resolveDeleteSemantics`(baseline→exclude·tenant→soft_delete·레거시 폴백). 스코프 가드 준수 — 뮤테이션 와이어링·필드 optional→required 전환·API 계약 변경 전부 금지, `action`→`is_overridden` 파생 로직(미확정)은 테스트 대상에서 제외. `npx vitest run` 17 passed, `npm run build`(tsc+vite) 에러 0.
- **2026-08-06 / TrustBuilder + Claude** — **baseline seed 구축** (`ICFR_baseline_seed_20260723.md`, ADR-0027). 엑셀을 baseline의 단일 원천으로 repo에 포함하고, 모든 환경이 동일 baseline을 재현하도록 seed를 정식 경로로 삼는다(2-A-2 이관 스크립트는 역할 종료). ①`backend/seeds/2026_설계평가_RCM_리스트.xlsx` + `seeds/seed_baseline.py` 신규 — 계층 순 삽입·id 매핑·어서션 7종 유니크 마스터 후 연결·단일 트랜잭션·재실행 가드·명시적 `--reset`(baseline 6 + instance 5테이블만, 구 스키마 미대상)·검증 출력(파싱 수 대조, instance 0건, 구 `controls` 전후 불변). ②**파서는 추출·복제 없이 그대로 재사용** — `_parse_rcm_sheet`(api/rcm)·`find_rcm_sheet`(services/excel_parser)가 이미 controls 비결합 순수 변환임을 확인, `rcm.py` 무변경(물리 이동은 2-A-4-3). ③`tests/test_seed_baseline.py` 8건 신규 — 격리 sqlite에서 파싱→삽입 경로 검증(건수 하드코딩 없이 불변식: 파싱수=삽입수, FK 체인 전수, 어서션 연결 전수 대조, baseline_version=1, instance 0건, 재실행 가드). ④`Dockerfile`에 `COPY seeds/`·`COPY scripts/` 추가 — 둘 다 이미지에 없어 2-A-2도 `docker cp`로 실행됐던 상태 해소. **실측**: 엑셀 데이터 8~100행 = **93 통제**(명세의 "95종"과 다름), 프로세스 8/하위프로세스 29/위험 85/어서션 연결 469. 현행 baseline 95 = 동일 93 + Phase 0 데모 시드 `P-SALES` 2건이며 **공통 93건은 18필드·어서션까지 차이 0건** — 명세 §0 전제("중간 95와 최종 95는 내용이 다르다")는 마스터 로컬에 해당하지 않고, `--reset`으로 잃는 실데이터는 없다. `--reset` 실행은 마스터 직접 수행(본 커밋은 스크립트까지). 기존 실패 3건은 2-A-4 부채로 미수정·기록만(13.5), upload-excel 다중 헤더행 미대응도 기록만(13.6).
- **2026-08-04 / Regina + Claude** — **상태 보드 실측 동기화** (`ICFR-PROMPT-state-board-sync.md`, 문서 전용, 코드/스키마/DB 무변경). read-only DB 조회로 로컬 실측 재확인: ①구 구조 카운트 processes 8/sub_processes 29/risks 85/risk_categories 7/controls 96/control_assertions 469(이전 기록 95/472 대비 소폭 변동 — 원인 미확인). ②신 구조 스키마 11테이블(baseline 6+instance 5) 전부 존재 확인 → **2-A-2 스키마 랜딩 완료**로 명확히 기록, 단 데이터는 여전히 전부 0건(백필 미완, 13.4 참조). ③`control_instances` 컬럼 전수 조회로 overlay 판별 구조 확정: `baseline_control_id`(nullable)+`action` 조합, **`is_overridden` 물리 컬럼은 없음**(기존 서술과 표현 차이 — envelope 필드는 resolver 계산 결과로 추정, 코드 미확인). ④신규 확인 대기 2건 13.4에 추가(백필 마이그레이션 일정, resolver의 action→is_overridden 매핑 실체) — **추정 서술 금지, 협업자 회신 대기**. 코드/스키마/DB 변경 없음(문서 단일 커밋).
- **2026-07-24 / Regina + Claude** — **RCM envelope 계약 optional→required 전환 + source 뱃지** (`ICFR-PROMPT-envelope-required.md`, ADR-0027, FE). 2-A-3 조회 전환(main, `0ea1cf5`) 반영해 흡수 계약을 조임 — 조회만, mutation 라우팅 미연결. ①`dto.ts` `ControlDto`의 `source`/`baseline_id`/`is_overridden` optional→required(`baseline_id`는 필드 존재 required·값은 nullable 유지). ②`sourceEnvelope.ts` `buildSourceEnvelope` fallback 주입 제거 — 필드 누락 시 개발모드 `Error` throw, 프로덕션은 콘솔 에러 1회+안전 최소값 렌더; process/sub_process/risk는 아직 legacy 목록 API라 `buildOptionalSourceEnvelope`(부재=정상) 유지. ③`SourceBadge.tsx` 신규 — `is_overridden`→재정의, `source==='tenant'`→테넌트, 그 외→기준, 목록·상세 공용. ④`rcmMutationLock.ts` 신규 — `RCM_MUTATION_LOCKED=true` 단일 플래그로 생성/수정/삭제/Excel업로드 버튼 disabled+툴팁(`백엔드 이관 작업 중...`), 2-A-4 연결 시 제거 대상 TODO 명시. 라우팅 헬퍼는 mutation 미연결 순수 함수 상태 유지. ⑤`mockData.ts` 30건 전부에 envelope 필드 추가(required 전환에 맞춤). `npm run build`(tsc+vite) 에러 0. **데이터 검증은 2-A-2 이관 실행 후 보류** — 로컬 DB `baseline_controls`/`baseline_control_assertions`/`control_instances`/`control_assertion_instances` 0건 확인(문서상 2-A-2 699행 이관 기록과 불일치, DB 이관은 백엔드 소관이라 본 세션에서 미실행). 라이브 카운트(95/37/73/1)·뱃지 렌더·잠금 UI 육안 확인·브라우저 콘솔 클린 확인은 13.4 대기 목록에 기록, 이관 확인 후 재검증 필요. 브랜치: feature/rcm-envelope-required.
- **2026-07-23 / TrustBuilder + Claude** — **RCM 조회 API resolver 전환 2-A-3** (`ICFR_rcm_search_2a3_20260723.md`, ADR-0027). 병행 구축 종료 — **기존 API 동작을 바꾸는 첫 단계**(회귀 위험). 전제(2-A-2 이관, baseline 699행)는 실측 확인(초기 측정이 stale였음, 재측정으로 9/30/86/7/95/472·instance 0건·커밋 33039f0 확인). ①`api/rcm.py` `search_controls` 재구현 — 기존 SQL 조인/WHERE/ORDER BY/LIMIT → `resolve_controls(db)` 결과 dict 위 메모리 필터·정렬·페이지네이션. 파라미터 전부 유지(q·owner·frequency·is_key_control·auto_manual·preventive_detective·risk_level·sub_process_code·process_code·assertion·sort_by·sort_order·skip·limit). 관계 필드·envelope는 resolver가 이미 채워 조인 불필요. None 안전 정렬((is None, 값) 튜플). ②`get_control` 상세 — resolve_controls에서 정체성 id 매칭, 없으면 404(목록과 id 체계 일치). ③`schemas/rcm.py` `ControlRead`에 source envelope 3필드 **flat** 추가(source/baseline_id/is_overridden, 기본값으로 legacy `GET /controls` model_validate 호환) → ControlSearchOut 상속. 중첩 wrapper 금지(Regina FE flat 계약 sourceEnvelope.ts 정합). ④미사용 import(`or_`·`selectinload`) 정리. matrix·bulk·excel·POST/PATCH/DELETE 미변경(2-A-4 범위). **라이브 검증**(재빌드 후): search 95건, 필터 process_code=EL/assertion=E/risk_level=SR = 37/73/1로 **legacy SQL과 동일**(교차검증), 정렬 asc/desc·페이지네이션 skip=90→5건·상세 200/없는id 404·envelope flat 정상. controls 95 불변, config.admin_password 불변. **⑤회귀 신호 보고→xfail 처리**: test_rcm.py 7건(test_control_extended_crud + search 6건)이 red — POST가 legacy `controls`에 쓰고 search/상세는 resolver(baseline/instance, 테스트 DB에선 비어있음)를 읽는 **write→read 소스 분리**가 원인(내 구현 버그 아님, READ를 WRITE보다 먼저 전환한 필연). 명세 §4 지시대로 테스트 로직 미수정, `@pytest.mark.xfail(strict=True, reason="...소스 분리")` 마커(`_XFAIL_SRC_SPLIT`)만 부착 — 2-A-4(POST→instance add) 전환 시 왕복 복구되면 strict가 xpass를 실패로 띄워 마킹 제거 신호. pytest **105 passed, 7 xfailed, 0 failed**(기존 회귀 없음). 후속: 2-A-4 CRUD 전환(+ xfail 마킹 제거). push는 마스터 직접.
- **2026-07-22 / Regina + Claude** — **RCM envelope 흡수 자리 준비 + flat 계약 정합** (`ICFR-PROMPT-envelope-adapter-prep.md`, ADR-0027 2-A-3 착륙 지점 사전 확보). 0단계 사전 확인(추정 금지): 어댑터 계층은 `dto.ts`(wire)/`controlsAdapter.ts`(→domain) 2파일에 process/sub_process/risk/control 4계층 공존, 별도 공통 타입 파일 없음 확인. ①`features/rcm/api/sourceEnvelope.ts` 신규 — `SourceEnvelope` 타입(id/source/baseline_id/is_overridden) + `isBaseline`/`isTenantAdd`/`resolveDeleteSemantics`(baseline→exclude, tenant→soft_delete, 부재 시 soft_delete 폴백) 순수 헬퍼. ②4계층 DTO·도메인 타입에 envelope 관련 필드 optional 추가. ③어댑터 4개 매핑 함수(`toControl`/`toProcessItems`/`toSubProcessItems`/`toRiskItems`)에 envelope 매핑 추가, UI·mutation·쿼리키 무변경. **머지 후 push 직전 원격 fetch에서 TrustBuilder의 2-B-4(`d325705`, `control_resolver.py`) 실 구현을 재확인하여 계약 불일치 발견** — 백엔드는 `source`/`baseline_id`/`is_overridden`을 resolved dict **최상위(flat) 필드**로 얹히고 `id`는 신규 필드가 아니라 기존 항목 id를 정체성 id로 재사용하는데, 최초 구현은 `envelope?: SourceEnvelope` **nested 객체**로 가정해 2-A-3 실 연결 시 항상 `undefined`가 될 뻔함(사용자 지적으로 push 전 발견·수정). ④DTO를 flat 3필드(`source?`/`baseline_id?`/`is_overridden?`, id는 기존 필드 재사용)로 수정, `sourceEnvelope.ts`에 `buildSourceEnvelope` 신규(flat 필드+기존 id → 도메인 nested `SourceEnvelope` 조립, `source` 부재 시 `undefined` 폴백·throw 없음) — 4계층 어댑터가 이 헬퍼 경유하도록 교체. `npm run build`(tsc) 통과(수정 전·후 각각 확인). 브랜치 없이 main 직접 작업(저위험 준비)  — `feature/rcm-envelope-adapter-prep` → main `--no-ff` 머지(b1d2766 태운 지점) 후 origin/main(2-B-4 등 5커밋) merge 통합, flat 정합 커밋(77aa28f) 추가 후 push. 실 API 연결은 2-A-3 이후.
- **2026-07-20 / Regina + Claude** — 다이얼로그 접근성 경고 해소 (`ICFR-PROMPT-dialog-a11y-fix.md`). 전 모듈 19개 Dialog/AlertDialog/Sheet에 `DialogDescription`/`AlertDialogDescription`/`SheetDescription` 추가(2곳은 기존 보유), 삭제 확인류는 "선택한 <대상>을 삭제합니다. 이 작업은 되돌릴 수 없습니다." 형태로 통일. 상세 조회 Sheet 4곳·통제 선택 다이얼로그는 `sr-only`로 레이아웃 변화 없이 처리. 동작·API 변경 없음. `npm run build` 에러 0, 브라우저 콘솔 `Missing Description` 경고 0건 확인(`Blocked aria-hidden`은 Radix 내부 이슈로 범위 외). 브랜치: fix/dialog-a11y → main `--no-ff` 머지 완료(5f2c828).
- **2026-07-20 / Regina + Claude** — **RCM 어댑터 경계 도입** (`ICFR-PROMPT-rcm-adapter-layer.md`, ADR-0027 2-B 착륙 지점 사전 확보). 목적: `controlsApi.ts`/`rawcApi.ts` 서버 응답이 훅 경유 컴포넌트까지 무가공으로 흐르던 구조에, 2-B-4 계약 확정 시 매핑을 흡수할 지점을 미리 한 곳에 모음 — **이번 작업에서 계약 변경 없음**(신규 필드 미추가). ①`features/rcm/api/dto.ts` 신규 — `ControlDto`/`ControlListResponseDto`/`ProcessItemDto`/`SubProcessItemDto`/`RiskItemDto`/`ControlRiskAssessmentDto`/`RawcListResponseDto`, 기존 `types.ts`(도메인 타입)와 필드 동일. ②`controlsAdapter.ts`/`rawcAdapter.ts` 신규 — DTO→도메인 1:1 명시적 필드 매핑(`as` 캐스팅 없음), 파일 상단에 "2-B 착륙 지점" 명시. ③`controlsApi.ts`/`rawcApi.ts` 응답 타입을 DTO로 교체(요청 payload 타입은 `types.ts` 유지), 함수 시그니처·엔드포인트 불변. ④`useControls`/`useRawcByControl`에 `select: toControlList`/`toRawcList` 연결(제네릭 `<Dto, Error, Domain>` 명시), queryKey·staleTime·placeholderData 불변, mutation 3종은 2-A-4 CRUD 스펙 대기로 미변경. ⑤`mockData.ts`(미사용 fixture) `Control`→`ControlDto` 참조 교체. 컴포넌트 파일(`features/rcm/components/**`, `pages/**`) 무수정. `npm run build` 에러 0. 브라우저 검증(로그인 → RCM 목록·필터·페이지네이션·RAWC 섹션 렌더링, `X-Tenant-Id` 헤더 유지, 콘솔 신규 에러 0 — 기존 React Router future flag·shadcn Dialog aria warning만 잔존) 통과. 사전 확인 중 원격에 2-B-1/2/3(baseline·instance·assertion junction) 커밋 4개 선반영 확인 — `control_resolver.py` diff로 응답 키 불변 재확인 후 진행. 브랜치: feature/rcm-adapter-layer → main `--no-ff` 머지 완료(d5d0871). 나머지 4개 모듈(evidence/remediation/test/users)은 2-B 스펙 확정 후 동일 패턴 일괄 적용 예정.
- **2026-07-16 / TrustBuilder + Claude** — **resolver 계층 확장 + source envelope 2-B-4** (`ICFR_rcm_baseline_2b4_20260716.md`, ADR-0027, **2-B 마지막 단계**, Regina FE 피드백 ② 통합). 구조(baseline5+instance4+junction2+baseline_version)를 실제 병합해 쓰는 로직 완성 — 2-A-3 조회 전환 선행조건 전부 해소. ①`services/control_resolver.py` 전면 확장: 공통 병합 함수 `_resolve_layer`(baseline−exclude+override(non-NULL, False 유효)+add, source envelope 부착) 1개로 4계층 처리(ADR-0020 "병합만 공통 함수"). `resolve_processes`/`resolve_sub_processes`/`resolve_risks` 신규 + `resolve_controls` 재작성 — 전부 tenant 인자 없음(자동격리). ②**정체성 id 규칙 계층 통일** `_parent_id` 헬퍼: 상위 참조 = instance_id 있으면 그것(add 상위)·baseline_id 있으면 그것(override 상위 포함)·둘 다 NULL이면 baseline 원래 상위 FK. resolved 항목 id·상위참조 모두 정체성 id → 메모리 lookup 체인(조인 아님). ③**source envelope**(id/source/baseline_id/is_overridden) process/sub/risk/control 전 계층 공통 — adopt=baseline/id/False, override=baseline/id/True, add=tenant/null/False. 프론트 단일 라우팅(2-A-4 편집·삭제 대상 판별 기반). 어서션은 코드배열이라 미적용. ④**cascade** 상위 exclude→하위 자동 제외(process→sub→risk→control alive 집합 전파). **단 control.risk_id NULL은 제외 안 함**(이관 전 risk 없는 통제 보존), risk 있는데 죽었을 때만 제외. ⑤**어서션 병합** `_resolve_assertions`: baseline_control_assertions − remove + add(정체성 id 매칭) → sorted 코드배열(`ControlSearchOut.assertions` 형태). ⑥**2-B-2 부채 정리** — 기존 `risk_id=inst.risk_baseline_id`(instance 참조 무시) 임시 매핑을 `_parent_id`로 교체(risk_instance_id resolve). ⑦**관계 필드**(risk_level/sub_process_code/process_code) resolve_controls가 항상 채움 — 목록 API(2-A-3) 단일 호출 목적, `api/rcm.py:331-340` selectinload 조인과 동일 결과. **API·기존 테이블·마이그레이션 미변경**(2-A-3 범위, alembic head `b8c9d0e1f2a3` 유지). ⑧테스트 7개 신규(계층 4 action+envelope, cascade+risk NULL 유지, add 체인 risk_id=instance id resolve, 어서션 remove+add, 관계 필드, override 상위 정체성 baseline 유지, control envelope) + 기존 adopt 키셋 단언 갱신(envelope+관계필드 반영) — pytest **112 통과**(기존 105 회귀 없음), control_resolver.py 커버리지 100%. `docker compose up -d --build backend` 재빌드·헬스체크 OK, 컨테이너 내 resolver import 확인, **controls 95건 불변**. config.admin_password 불변. 후속: 2-A-2 기존 controls→baseline/instance 이관 → 2-A-3 조회 API 전환 → 2-A-4 CRUD 전환.
- **2026-07-16 / TrustBuilder + Claude** — **baseline_version + 삭제 의미론 2-B-3.5** (`ICFR_rcm_baseline_2b35_20260716.md`, ADR-0027, Regina FE 피드백). baseline 개정 이력 선반영("지금은 컬럼 하나, 나중은 프로젝트 하나" — 원칙 7). ①`models/rcm_baseline.py` baseline **5테이블**(processes/sub_processes/risks/risk_categories/controls)에 `baseline_version: Mapped[int]`(Integer, default=1, NOT NULL) 추가 — **행 단위**(전역 버전 아님: 법령 부분 개정 대응, 개정된 행을 쓰는 tenant만 영향). instance 미도입(소급 매핑은 개정 트랙 미결). `VersionMixin.row_version`(낙관적 잠금)과 **별개 개념**, 이름 충돌 없음 — 각 컬럼·모듈 docstring 명시. ②**삭제 의미론 규약** 모듈 docstring 명문화(전 계층 공통, 계층별 분기 금지): tenant는 baseline 물리삭제 불가 → "삭제"=exclude instance(어서션은 remove action, 원본 보존), 자기 add 삭제 → soft delete(is_deleted). resolver(2-B-4)가 exclude/remove + add soft-delete 제외. 실 CRUD는 2-A-4, resolver 반영은 2-B-4 — 본 단계는 컬럼+규약 문서화까지. ③resolver 무변경 — `control_resolver.py`는 고정 `CONTROL_FIELDS`만 순회하므로 baseline_version이 응답 키에 새어들지 않음(기존 응답 형태 테스트 회귀 없음 확인). ④마이그레이션 `b8c9d0e1f2a3`(down_revision=`a7b8c9d0e1f2`) — 5컬럼을 `server_default='1'`로 추가(기존 행 backfill) 후 `server_default=None` 제거(모델 app-side default와 일치). 실 Postgres 적용: 5테이블 NOT NULL 확인, downgrade→upgrade 왕복(5→0→5 컬럼) 검증, **controls 95건 불변**. ⑤테스트 2개 신규(5테이블 기본값=1 + 명시적 version 설정 가능) — pytest **105 통과**(기존 103 회귀 없음). `docker compose up -d --build backend` 재빌드·헬스체크(`/api/health/` status:ok) OK. config.admin_password 불변. 후속: 2-B-4 resolver 확장(어서션 병합 + baseline_version 반영은 개정 트랙에서).
- **2026-07-16 / TrustBuilder + Claude** — **어서션 junction baseline/overlay 2-B-3** (`ICFR_rcm_baseline_2b3_20260716.md`, ADR-0027). 기존 control_assertions(472건)·기타 테이블 미변경 병행 구축. ①`models/rcm_baseline.py`에 `BaselineControlAssertion`(IdentityBase 전역 — baseline_control_id+baseline_risk_category_id 복합 unique로 중복 연결 차단, `BaselineControl.assertions` back_populates) + `ControlAssertionInstance`(AuditedBase — tenant_id 자동) 신규. ②**junction은 2 action** — 연결은 있거나 없거나이므로 add(표준에 없던 연결 추가)/remove(표준 연결 끊음)만. baseline 연결에 remove 없음=암묵 채택(resolver 반영은 2-B-4). ③대상 통제는 이중 nullable FK(`control_baseline_id`/`control_instance_id`) + `ck_control_assertion_instances_single_parent` check — 2-B-2 규칙 동일(override 통제 정체성=baseline). 어서션 쪽은 `baseline_risk_categories` **단일 FK** — RiskCategory baseline-only(2-B-1 결정)이므로 이중 불필요. ④복합 unique 2개(tenant+통제참조+어서션)로 같은 연결에 두 결정 차단 — 단 NULL 포함 복합 unique는 Postgres에서 무력하므로 실질 중복 차단은 앱 레벨 검증 병행(모델 docstring 명시). ⑤마이그레이션 `a7b8c9d0e1f2`(down_revision=`f6a7b8c9d0e1`) — 신규 2테이블만. 실 Postgres 적용·downgrade 왕복 검증, **controls 95건·control_assertions 472건 불변**. ⑥테스트 5개 신규(표준 연결+중복 차단, add/remove, add 통제 대상 연결, 이중 FK check 위반, tenant 격리—baseline 연결은 전역) — pytest **103 통과**(기존 98 회귀 없음). `docker compose up -d --build backend` 재빌드·헬스체크 OK. config.admin_password 불변. 후속: 2-B-4 resolver 확장(어서션 병합: baseline−remove+add, 응답은 ControlSearchOut.assertions 코드 배열 호환).
- **2026-07-16 / TrustBuilder + Claude** — **RCM instance 상위 계층 2-B-2** (`ICFR_rcm_baseline_2b2_20260716.md`, ADR-0027). 기존 processes/sub_processes/risks/controls 미변경 병행 구축. ①`models/rcm_baseline.py`에 `ProcessInstance`·`SubProcessInstance`·`RiskInstance` 3모델 신규 — 전부 AuditedBase(tenant_id 자동), ControlInstance 패턴 그대로(UniqueConstraint 2개 + Index + action + nullable 미러링). ②**이중 nullable FK 상위 참조**: `<상위>_baseline_id`(baseline 상위 밑) / `<상위>_instance_id`(add한 상위 밑) — override된 상위의 정체성은 baseline이므로 baseline_id 사용, 둘 다 NULL=baseline 상위 따름, 둘 다 non-NULL은 `CheckConstraint`(ck_*_single_parent)로 차단. add 시 "상위 참조 하나 필수"는 앱 레벨 검증(명세 §5, DB check로 표현 곤란). ③`ControlInstance.risk_id`(FK→risks) 제거 → `risk_baseline_id`/`risk_instance_id` 이중 FK 전환 — 계층 참조 방식 통일(2-B-4 resolver 일관 규칙 목적). 전환 전 control_instances **0건 확인**. `BaselineControl.risk_id`(→baseline_risks)는 미변경. ④`control_resolver.py` 최소 수정(확장은 2-B-4): `_MERGE_FIELDS`=CONTROL_FIELDS−risk_id 분리 — instance에 risk_id가 없어져 AttributeError 방지. 응답 키 구성(CONTROL_FIELDS+id/created_at/updated_at)은 불변, override는 risk_baseline_id non-NULL 시 risk_id로 반영, add 행 risk_id=risk_baseline_id(instance 참조 resolve는 2-B-4). ⑤마이그레이션 `f6a7b8c9d0e1`(down_revision=`e5f6a7b8c9d0`) — 3테이블 신규+check, control_instances risk_id drop→이중 FK+check 추가. 실 Postgres 적용·downgrade 왕복(3테이블 drop + risk_id FK 복원 → 재생성) 검증, **controls 95건 불변**. ⑥테스트 6개 신규(계층별 4 action 규칙 + baseline/instance 상위 참조 + 이중 FK check 위반 차단 3계층 parametrize + tenant 격리) — pytest **98 통과**(기존 92 회귀 없음). `docker compose up -d --build backend` 재빌드·헬스체크 OK. config.admin_password 불변. 후속: 2-B-3 어서션 junction overlay → 2-B-4 resolver 확장.
- **2026-07-16 / TrustBuilder + Claude** — **RCM baseline 상위 계층 2-B-1** (`ICFR_rcm_baseline_2b1_20260716.md`, ADR-0027). 기존 processes/sub_processes/risks/risk_categories 미변경 병행 신규 구축. ①`models/rcm_baseline.py`에 `BaselineProcess`·`BaselineSubProcess`·`BaselineRisk`·`BaselineRiskCategory` 4모델 신규 — 전부 IdentityBase(전역, tenant 비종속), code 전역 unique, 기존 rcm.py 필드 미러링. FK 체인 baseline_sub_processes→baseline_processes, baseline_risks→baseline_sub_processes + 관계 미러링. **RiskCategory(어서션)는 baseline만, instance 미도입** — 제도 고정 개념, 필요 시 후속에서 instance만 추가(되돌리기 쉬운 쪽 선택). ②`BaselineControl.risk_id` FK 전환: bare UUID → FK→baseline_risks.id nullable+index (2-A-1 위임 이행. nullable은 2-A-2 이관 후 NOT NULL 검토). `ControlInstance.risk_id`는 미변경(tenant 종속 — 기존 risks FK 유지, 이중 참조 설계는 2-B-2). ③마이그레이션 `e5f6a7b8c9d0`(down_revision=`d4e5f6a7b8c9`) — 4테이블 신규 + FK 제약·인덱스만 추가. FK 추가 전 baseline_controls **0건 확인**(제약 충돌 없음). 실 Postgres 적용·downgrade 왕복(FK 제거+4테이블 drop→재생성) 검증, **controls 95건 불변**. ④테스트 2개 신규(FK 체인 4단 조인으로 control→process code 조회 + 전역성: 타 tenant·무컨텍스트 조회) — pytest **92 통과**(기존 90 회귀 없음). `docker compose up -d --build backend` 재빌드·헬스체크 OK. config.admin_password 불변. 효과: baseline FK 체인이 전부 전역이라 tenant 필터에 안 걸림 → 2-A-3 조회 전환(관계 필드·검색 필터) 선행조건 해소. 후속: 2-A-2 이관 → 2-A-3 조회 전환, 2-B-2 상위 계층 instance.
- **2026-07-14 / Regina + Claude** — **`X-Tenant-Id` 헤더 주입** (`ICFR-PROMPT-tenant-header-axis.md`, 멀티테넌시 후속 ③). 목적: tenant 프리픽스를 붙일 지점을 미리 모아둔 queryKey 팩토리(2026-07-13)를 실제로 채워 캐시 격리 구조를 완성. tenant 1개(사이냅소프트/DEFAULT)뿐인 현재는 **동작 변화 0**. ①**STEP1 실물 확인**: `POST /api/auth/login`(form-urlencoded) → `GET /api/auth/me` 실호출로 `tenants: [{id,name,code,role}]`(id는 UUID 문자열) + `active_tenant_id`(UUID 문자열) 확인, 기존 필드(id/email/display_name/role/is_active/created_at/updated_at) 회귀 없음. ②`features/auth/store.ts`: `TenantSummary` 타입 신규 + `UserProfile`에 `tenants`·`active_tenant_id` 추가. `useActiveTenantId()` 훅 신규 — 소스는 오직 `user.active_tenant_id`(JWT 파싱·별도 localStorage 없음, 단일 소스 원칙). ③`lib/axios.ts`: 기존 토큰 인터셉터 옆에 두 번째 request 인터셉터 추가 — `useAuthStore.getState().user?.active_tenant_id`를 모듈 스코프에서 직접 읽어(React hook 비의존) `X-Tenant-Id` 헤더 주입. `/auth/login`·`/auth/refresh`·`/auth/me`는 URL 패턴으로 명시 제외. tenantId 없으면(로그인 직후 `/me` 이전) 헤더 없이 통과, 에러 처리 없음. ④`lib/queryKeys.ts`: 5개 모듈(RCM/Test/Evidence/Remediation/Users) 전 함수 시그니처에 `tenantId`(첫 인자) 추가 — 반환 키를 `['tenant', tenantId ?? 'no-tenant', '<module>', ...기존키]`로 변경. tenantId `null`이어도 `'no-tenant'` sentinel로 키 안정성 유지. ⑤9개 소비 파일(`useControls`·`useRawc`·`BasicInfoTab`·`useTestRuns`·`useEvidence`·`useRemediationPlans`·`useDeficiencies`·`useUsers`·`useUserRoles`) 전부 각 훅 내부에서 `useActiveTenantId()`로 읽어 `queryKeys.*` 호출에 전달 — **컴포넌트 호출부(예: `useControls(params)`) 시그니처 변경 없음**. invalidateQueries 전수 재점검 — 하드코딩 키 리터럴 0건, 전부 factory 경유 확인. `npm run build` 통과. 브랜치: feature/tenant-header-axis → main `--no-ff` 머지 완료(917d9b5). 후속: ②tenant 생성/온보딩 API + 회사 전환 UI는 다음 세션.
- **2026-07-13 / Regina + Claude** — queryKey 팩토리 중앙화 (`ICFR-PROMPT-query-key-factory.md`). 목적: 멀티테넌시 Phase 2에서 tenant 프리픽스를 붙일 지점을 한 곳으로 모아 retrofit 방지, 이번 작업의 동작 변화는 0. `frontend/src/lib/queryKeys.ts` 신규 — RCM(`controls`·`rawc`·`processes`·`sub-processes`·`risks`)·Test(`testRuns`·`testRunDetail`·`testRunHistory`·`testSteps`)·Evidence(`evidence-files`)·Remediation(`remediationPlans`·`remediationPlanDetail`·`remediationPlanHistory`·`deficiencies`)·Users(`users`·`userRoles`) 5개 모듈 키를 계층 팩토리로 이동, 기존 키 문자열 100% 보존(임의 rename 없음). `// TODO(tenant)` 주석으로 tenant_id prepend 지점 표시. 9개 훅 파일(`useControls`·`useRawc`·`BasicInfoTab`·`useTestRuns`·`useEvidence`·`useRemediationPlans`·`useDeficiencies`·`useUsers`·`useUserRoles`)의 인라인 queryKey·invalidateQueries 배열 리터럴 전부 팩토리 참조로 교체. `setQueryData`/`removeQueries` 사용처 0건 확인. 범위 외 `auth/useAuth.ts`의 `['me']`는 보존(5개 모듈에 미포함). 빌드 통과, 브라우저 확인 — 5개 모듈 CRUD 후 목록 자동 갱신 전부 정상(새로고침 필요한 화면 없음, invalidation 키 불일치 0건). 브랜치: refactor/query-key-factory → main `--no-ff` 머지 완료. **선행 작업으로 `X-Tenant-Id` 헤더 주입(`ICFR-PROMPT-tenant-header-axis.md`) 0번 선행확인 결과도 기록**: 이날 오전 시점 로그인 응답·JWT 클레임(`sub`/`exp`/`type`만 존재)·`/api/auth/me` 응답(`id`/`email`/`display_name`/`role`/`is_active`/`created_at`/`updated_at`) 어디에도 tenant_id 없어 구현 중단. 임의 `X-Tenant-Id` 헤더 테스트: 형식 오류 400, 형식은 맞으나 미권한 UUID 403 (백엔드 `deps.py`가 `UserTenantAccess` 테이블로 서버측 자동 해석 중이라 FE가 tenant_id를 확보할 경로 자체가 없었음). **같은 날 아래 TrustBuilder 커밋으로 `/me`에 tenant 정보가 추가되어 재개 가능** — 후속은 섹션 13.3 참조.
- **2026-07-13 / TrustBuilder + Claude** — **RCM baseline/instance 2-A-1** (`ICFR_rcm_baseline_2a1_20260615.md`, ADR-0027). 기존 controls 미변경 병행 신규 구축. ①`models/rcm_baseline.py` 신규: `BaselineControl`(IdentityBase — tenant 비종속, code 전역 unique) + `ControlInstance`(AuditedBase — tenant_id 자동, action=adopt/exclude/override/add, baseline 전 필드 nullable 미러링, `(tenant_id, code)`·`(tenant_id, baseline_control_id)` 복합 unique). **risk_id 결정(명세 위임)**: baseline.risk_id=FK 없는 nullable UUID(전역→tenant 종속 risks FK는 격리 위반, 2-B baseline_risks에서 FK 전환), instance.risk_id=FK→risks nullable. ②`services/control_resolver.py` `resolve_controls(db)`: baseline−exclude+override병합+add, 반환 dict=ControlRead와 동일 키. tenant 인자 없음 — ADR-0025 자동격리 ContextVar 사용(수동 필터 금지). override의 False값 유효 처리(`is not None` 판정). adopt/override 행 id=baseline id, add 행 id=instance id(2-A-4에서 재검토). ③마이그레이션 `d4e5f6a7b8c9`(down_revision=`c3d4e5f6a7b8`) — 신규 테이블 2개만. 실 Postgres 적용·downgrade 왕복(두 테이블 drop→재생성) 검증, **controls 95건 불변 확인**. ④테스트 6개 신규(4 action 각각+혼합+tenant 격리) — pytest **90 통과**(기존 84 회귀 없음). `docker compose up -d --build backend` 재빌드·헬스체크 OK. config.admin_password 불변. 후속: 2-A-2 이관 → 2-A-3 조회 전환 → 2-A-4 CRUD 전환.
