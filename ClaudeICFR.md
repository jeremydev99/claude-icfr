...
1. 컨트롤 목록 라이브 카운트 95건 / `process_code=EL` 37건 / `assertion=E` 73건 / `risk_level=SR` 1건
   ...
   - 1-b. 라이브 API 카운트 검증(`GET /controls/search` 실호출, `process_code=EL`/`assertion=E`/`risk_level=SR` 필터별 건수) — **미완**, 새 기준값(93 기반) 재산정 후 확인 필요.
