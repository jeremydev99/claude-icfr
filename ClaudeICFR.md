...
3. **멀티테넌시 후속 (ADR-0026 미결)**: ~~①`code` 등 글로벌 unique → `(tenant_id, code)` 복합 unique 전환~~ ✅ 완료 (2026-07-08, ADR-0026 추가결정 참조). ②tenant 생성/온보딩 API + 회사 전환 UI. ③프론트엔드 `X-Tenant-Id` 헤더 주입 — 2026-07-13 오전 선행확인 시점엔 로그인 응답·JWT·`/me` 어디에도 tenant_id 없어 **중단**했으나, 같은 날 백엔드가 `/me`에 `tenants`·`active_tenant_id` 추가(아래 항목)로 **재개 가능**해짐. ~~③-선행: queryKey 팩토리 중앙화~~ ✅ 완료 (2026-07-13) — tenant 프리픽스 붙일 지점을 한 곳(`src/lib/queryKeys.ts`)으로 모아둠. 다음 세션에서 `TenantContext`+헤더 인터셉터 이어서 진행.
