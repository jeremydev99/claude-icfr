...
### 13.5.1 xfail 현황 (6건, 전부 `_XFAIL_SRC_SPLIT`)

2-A-4-2 후 남은 xfail은 전부 **동일 근본원인** — write가 legacy로 가는데 read는 resolver를 보는 소스 분리. 2-A-4-3에서 일괄 해소된다.
...
| `test_search_response_includes_assertions` | `POST /control-assertions`가 legacy junction에 씀 |
