...
10. **Docker Hub 익명 pull 한도. 서버 공인 IP 단위로 걸린다** (2026-09-18, Deploy #38·#39 연속 실패).
    ...
    **다이제스트 고정만으로는 부족하다.** `compose pull` 은 **로컬 존재 여부와 무관하게 전 서비스를
    레지스트리에서 조회한다.** 고정의 이점은 `up -d` 에서만 나오므로, 배포 스크립트의 pull 대상을
    **자사 이미지로 한정해야** Docker Hub 조회가 0회가 된다
    (`deploy.yml`: `docker compose ... pull backend frontend`).
    2026-09-18 다이제스트 고정 후에도 **Deploy #41 이 같은 지점에서 실패해 확인됨** —
    로그에는 `Image minio/minio@sha256:14cea49… Pulling` 으로 다이제스트가 정상 해석된 채
    같은 `pull access denied` 가 났다. **고정만 하고 pull 을 그대로 두면 증상이 하나도 바뀌지 않는다.**
