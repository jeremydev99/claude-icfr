...
> **스냅샷 문서 — 기준 커밋 `4b1d912` / 2026-09-09 시점, 2026-09-22 13.9-48 권한 수정 반영(§3.1). API 변경 시 갱신 필요.**
>
...
>
> 참조 표기 — 문서명 없는 `§4.1` 은 이 문서의 절이다. 다른 문서는 `ADR-0032 §2.7`,
> `org-contract.md` §5.0, `ClaudeICFR.md` 13.9-29 처럼 문서명을 붙인다.
...
통제×회차 직접 부착으로 바뀌었기 때문이다(§6). 용도 정리는 별건(`ClaudeICFR.md` 13.9-30).
**연결 생성·삭제는 `icfr_manager` 전용이다**(2026-09-22, 13.9-48) — §3.1.
...
**권한 판정은 두 축이다** — 회차 상태 × 통제 단위 역할(ADR-0032 §2.7).
...
### 3.1 2026-09-22 권한 수정 (13.9-48)

그 전에는 아래 경로가 **로그인만 확인**했거나 `require_write` 만 적용됐다. 외부감사인이 증빙 연결을
만들고 지우고, 파일 레코드의 저장 경로를 바꿀 수 있었다 — 감사 증거 훼손 경로.

| 경로 | 권한 | 비고 |
|---|---|---|
| `POST /api/evidence/links` | `icfr_manager` 만 | 대상 종류 허용 목록(`control`) 밖 → 422, 없는 통제 → 404, 없는·삭제된·다른 회사 파일 → 404 |
| `DELETE /api/evidence/links/{id}` | `icfr_manager` 만 | 소프트 삭제 + `deleted_by`(사용자 id)·`deleted_at`. 생성은 `created_by`. 사유 칸 없음(별건) |
| `PATCH /api/evidence/files/{id}` | 업로드와 같은 판정(위 표 + `external_auditor` 거부) | **바꿀 수 있는 것은 `filename` 뿐.** `minio_key` 등 다른 필드를 보내면 422 |
| `DELETE /api/evidence/files/{id}` — 레거시(회차 없음) | `icfr_manager` 만 | 문구 `"회차가 없는 기존 증빙은 내부회계관리자만 수정·삭제할 수 있습니다"` |

조회(`GET`)는 전부 인증만 — 바뀌지 않았다.

## 4. 삭제와 이력

### 4.1 삭제해도 파일이 남는다

**DB 레코드도, MinIO 파일도 지우지 않는다**(ADR-0032 §2.7 "삭제는 물리 삭제가 아니다").
삭제 표시만 하고 조회에서 제외한다.

레코드만 남기고 파일을 지우면 "그때 지운 게 뭐였나"에 답할 수 없다. 보존기간이
5년 이상이므로 파일도 그 기간을 따른다(ADR-0032 §2.9).
...
소진한다.** 스트리밍 전환이 선행되어야 한다(`ClaudeICFR.md` 13.9-31). 그때까지 상한을 유지하되
...
NOT NULL 로 만들 수 없었고, 그 데이터를 조작하지 않기로 확정했다(`ClaudeICFR.md` 13.9-29).
...
조회가 DB 레코드의 `minio_key` 를 쓰므로 신규 규칙과 충돌하지 않는다(`ClaudeICFR.md` 13.9-29).
...
**삭제 후 MinIO 파일이 실제로 남는지(§4.1)는 대역 검증만 됐다.**
...
§4.1 이 실증된다(`ClaudeICFR.md` 13.9-33).

```bash
docker exec icfr-minio mc ls -r local/icfr-evidence
```

그 밖의 동작 — 통제×회차 필수(§6), 저장 경로와 파일명 분리(§2.1), 403 3종과
`external_auditor` 거부(§3), 삭제 이력(§4), 보존기간 5년 하한(§5) — 은
`tests/test_evidence_cycle.py`·`tests/test_evidence_retention.py` 로 로컬에서
통과했다(`ruff` All checks passed,
